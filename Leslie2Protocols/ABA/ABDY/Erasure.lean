...
the round's bound bit, written by `Net.abdyGhostStep` and announced on every
graded-agreement return by `Net.abdyAnnouncedBound`. `ABDY.protocol₀` is the protocol as
...
admits an announced bit, which here is the equation `bnd = Net.abdyGhostOut P s r id out`
...
noncomputable def protocol₀ (P : Params) :
    System (Net.FlatState P GBCA.Msg (GBCA.StageRec P.n) Unit) (Lab P.n) :=
...
theorem protocol_erasure (P : Params) :
  ...
  Net.flat_erasure P GBCA.Msg (GBCA.StageRec P.n) (Option Bool) (Net.AbdyStageStep P)
    (Net.gCallPayload P) (Net.abdyGhostStep P) (Net.abdyAnnouncedBound P)
    (fun s r id out => ⟨Net.abdyGhostOut P s r id out, rfl⟩)
