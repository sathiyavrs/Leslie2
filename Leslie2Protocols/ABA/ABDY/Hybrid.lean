...
`ABA/ABDY/Protocol.lean` presents the protocol as `n` programs beside one network
...
the ABA-side network and the lifted oracle are defined in `ABA/ABDY/Components.lean`,
the round instances in `ABA/ABDY/Instances.lean`; the composition pipeline
...
of `ABA/ABDY/Components.lean` and `ABA/ABDY/Instances.lean`.

The core simulation of `ABA/Core/Sim.lean` runs from `hybrid` on this
vocabulary, and the non-vacuity witnesses of `ABA/Core/NonVacuity.lean` are built
