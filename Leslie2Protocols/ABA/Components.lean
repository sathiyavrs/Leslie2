...
(`ABA/NetAlphabet.lean`); `NetEvt n` is that alphabet at the stage messages of
`ABA/GBCAImpl.lean`, and `NLab n = Lab n ⊕ NetEvt n` is the alphabet every
...
graded-agreement message type (`ABA/NetAlphabet.lean`). This reading fixes that
type to the stage messages of `ABA/GBCAImpl.lean`. -/

/-- The rendezvous alphabet at the stage messages of `ABA/GBCAImpl.lean`. -/
