...
(`ABA/Implementation/Alphabet.lean`); `NetEvt n` is that alphabet at the stage messages of
`GBCA/ABDY/Implementation.lean`, and `NLab n = Lab n ⊕ NetEvt n` is the alphabet every
...
(`ABDY.ABAProcStepN`), whose record is the pair.
...
abbrev NetEvt (n : ℕ) : Type := NetEvtP n GBCA.ByABDY.Msg
...
theorem aStep_gsnd_noStep {r : ℕ} {k : Fin P.n} {m : GBCA.ByABDY.Msg}
    (h : ANetStep P a (Sum.inr (.gsnd r k m)) μ) : False := by cases h

theorem aStep_gdlv_noStep {r : ℕ} {i k : Fin P.n} {m : GBCA.ByABDY.Msg}
