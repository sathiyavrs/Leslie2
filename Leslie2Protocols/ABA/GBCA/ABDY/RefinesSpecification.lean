...
* `bound_excluded` — the round's bound bit and the specification's `excluded`
  ...
  (`excludedCert_of_noCorrectVote`) certifies both bits at once.
...
anywhere, its `vote_conf` receipt quorum is Case A for the opposite bit; otherwise the honest vote
prefix is all-⊥ and the `VoteWall` holds for both
bits at once.
...
`Inv.supp_of_input_receipts` for the `f + 1` InputSupport count (D15). At the
...
* the corruption budget (`F_card`) and delivery soundness (`recv_sub`); * protocol conformance of
honest multicasts (`echo_conf`, `vote_input`,
...
def ImplSupp (P : Parameters) (s : ImplementationState P.n) (b : Bool) : Prop :=
...
theorem ImplSupp.mono {s s' : ImplementationState P.n} {b : Bool}
...
structure Inv (P : Parameters) (s : ImplementationState P.n) : Prop where
  /-- Corruption budget. -/
  F_card : s.F.card ≤ P.f
  /-- Delivery soundness: everything delivered was multicast. -/
  recv_sub : ∀ i j m, m ∈ s.received i j → m ∈ s.sent j
  /-- Honest `ECHO b` is backed by an `n − f` `INPUT b` receipt quorum. -/
  echo_conf : ∀ j b, j ∉ s.F → Message.echo b ∈ s.sent j →
  ...
  vote_conf : ∀ j b, j ∉ s.F → Message.vote (some b) ∈ s.sent j →
  ...
  bind_conf : ∀ j b, j ∉ s.F → Message.bind (some b) ∈ s.sent j →
    P.n - P.f ≤ s.receivedCount j (.vote (some b))
  /-- Honest `BIND ⊥` is backed by `n − f` any-payload `VOTE` receipts. -/
  bindBot_conf : ∀ j, j ∉ s.F → Message.bind none ∈ s.sent j →
  ...
  echo5Bot_conf : ∀ j, j ∉ s.F → Message.echo5 none ∈ s.sent j →
  ...
  input_supp : ∀ (b : Bool) (j : Fin P.n), j ∉ s.F → Message.input b ∈ s.sent j →
    (s.process j).input = some b ∨ ImplSupp P s b
...
theorem Inv.initial (P : Parameters) : Inv P (ImplementationState.initial P.n) where
  F_card := by
    simp [ImplementationState.initial]
  recv_sub := fun i j m h => absurd h (by simp [ImplementationState.initial])
  echo_conf := fun j b _ h => absurd h (by simp [ImplementationState.initial])
  ...
  vote_conf := fun j b _ h => absurd h (by simp [ImplementationState.initial])
  ...
  bind_conf := fun j b _ h => absurd h (by simp [ImplementationState.initial])
  bindBot_conf := fun j _ h => absurd h (by simp [ImplementationState.initial])
  ...
  echo5_conf := fun j b _ h => absurd h (by simp [ImplementationState.initial])
  echo5Bot_conf := fun j _ h => absurd h (by simp [ImplementationState.initial])
  input_orig := fun b G _ _ j _ h => absurd h (by simp [ImplementationState.initial])
  input_supp := fun b j _ h => absurd h (by simp [ImplementationState.initial])
...
theorem Inv.supp_of_input_receipts {s : ImplementationState P.n} (hI : Inv P s)
...
private theorem Inv.send {s : ImplementationState P.n} (hI : Inv P s) {j : Fin P.n}
    ...
    Inv P ((s.setProcess j p).multicast j m) := by
  ...
  · -- recv_sub
    ...
    exact ImplementationState.sent_subset_multicast _ _ _ _ (hI.recv_sub i' j' m' hm')
  · -- echo_conf
    intro j' b hF hm'
...
private theorem Inv.setProcess_frame {s : ImplementationState P.n} (hI : Inv P s)
...
private theorem Inv.setBound {s : ImplementationState P.n} (hI : Inv P s) (β : Bool) :
    Inv P (s.setBound β) := { hI with }

/-- **Invariant preservation.** `Inv` is preserved by every implementation
step. -/
theorem Inv.step {r : ℕ} {s : ImplementationState P.n} {l : Label P.n}
    {μ : PMF (ImplementationState P.n)} {s' : ImplementationState P.n} (hI : Inv P s)
    (hstep : ImplementationStep P r s l μ) (hs' : s' ∈ μ.support) : Inv P s' := by
  cases hstep with
  | call id b h =>
  ...
  | retA id v bnd _hin _hlv hcnt hr _hbnd =>
    ...
    exact hI.setProcess_frame rfl rfl rfl rfl rfl
...
theorem echoQuorum_of_vote_receipts {s : ImplementationState P.n} (hI : Inv P s)
...
theorem inputQuorum_of_echoQuorum {s : ImplementationState P.n} (hI : Inv P s)
...
theorem echoQuorum_unique {s : ImplementationState P.n} (hI : Inv P s) {v v' : Bool}
...
`sentVote` field. -/
private theorem excludedCert_send {s : ImplementationState P.n} {j : Fin P.n} {p : ProcessRecord}
    {m : Message} (hvote : ∀ w, (s.process j).sentVote = some w → p.sentVote = some w)
    {b : Bool} (h : ExcludedCert P s b) :
    ExcludedCert P ((s.setProcess j p).multicast j m) b :=
...
private theorem excludedCert_ret {s : ImplementationState P.n} {id : Fin P.n} {b : Bool}
    (h : ExcludedCert P s b) :
    ExcludedCert P (s.setProcess id { s.process id with returned := true }) b :=
  ExcludedCert.mono (s := s) (fun _ _ _ hm => by simpa using hm)
    (fun k w hk => by
      by_cases hkj : k = id
      · subst hkj
...
bit). -/
private theorem excludedCert_setBound {s : ImplementationState P.n} {b β : Bool}
...
theorem excludedCert_of_noCorrectVote {s : ImplementationState P.n} (hI : Inv P s)
...
certifies *some* excluded bit. Classical dichotomy on "an honest bit-voter
exists somewhere": if yes, its `vote_conf` receipt quorum is Case A for the
opposite bit; if no, the all-⊥ wall of `excludedCert_of_noCorrectVote`
answers. -/
theorem excludedCert_of_echo5Bot_quorum {s : ImplementationState P.n} (hI : Inv P s)
  ...
  · exact ⟨false, excludedCert_of_noCorrectVote hI h hcase false⟩
...
theorem excludedCert_boundOf_C {s : ImplementationState P.n} (hI : Inv P s)
    {i : Fin P.n} (h : P.n - P.f ≤ s.receivedCount i (.echo5 none)) :
  ...
  · refine excludedCert_of_noCorrectVote hI h (fun hc => ?_) _
...
def instRel (P : Parameters) (_r : ℕ) (s : ImplementationState P.n) (t : SpecState P.n) : Prop :=
  InstRel P s t

/-- The initial states are related. -/
theorem instRel_init (P : Parameters) (r : ℕ) :
    instRel P r (implementation P r).init (specInst P r).init where
  inv := Inv.initial P
  call_eq := fun _ => rfl
  ret_eq := fun _ => rfl
  F_eq := rfl
...
certified. -/
theorem InstRel.bound_cert {s : ImplementationState P.n} {t : SpecState P.n}
...
theorem InstRel.retBound_eq {s : ImplementationState P.n} {t : SpecState P.n}
    (hR : InstRel P s t) {i : Fin P.n} {v : Bool} {out : GBCAOutput}
...
theorem suppI_of_valid {s : ImplementationState P.n} (hI : Inv P s)
...
theorem InstRel.spec_supp {s : ImplementationState P.n} {t : SpecState P.n}
...
theorem quorum_of_msg_quorum {s : ImplementationState P.n} {t : SpecState P.n}
    (hR : InstRel P s t) {i : Fin P.n} {m : Message}
...
theorem grade_ne_false_of_echo5_quorum {s : ImplementationState P.n} {t : SpecState P.n}
  ...
  obtain ⟨j, hjF, hj1, hj2⟩ := ImplementationState.exists_correct_received₂ hR.inv.F_card hcnt hc
...
theorem grade_ne_true_of_echo5Bot_quorum {s : ImplementationState P.n} {t : SpecState P.n}
  ...
  obtain ⟨j, hjF, hj1, hj2⟩ := ImplementationState.exists_correct_received₂ hR.inv.F_card hc hcnt
...
theorem refinesSpecification (P : Parameters) (r : ℕ) :
    ForwardSimulation (implementation P r) (specInst P r) (instRel P r) := by
  constructor
  ...
  | retC id bnd _hin _hlv _hnotA _hnotB hcnt hval hr hbnd =>
    ...
    | none =>
      ...
      have hbv : bnd = boundOf q1.sent q1.F .C := by
        rw [hbnd, hb]; rfl
  ...
  | fail id =>
    ...
    · intro b hb
      ...
      refine ExcludedCert.mono (s := q1) (fun i' j' m' hm' => ?_) (fun k w hk => ?_)
        (ImplementationState.corrupt_F_subset q1 id) (hRR.excluded_cert b hb)
      · rw [ImplementationState.corrupt_received]
        exact hm'
      · rw [ImplementationState.corrupt_process]
        exact hk
...
theorem instRel_corrupt (P : Parameters) (r : ℕ) (id : Fin P.n)
    {x : ImplementationState P.n} {y : SpecState P.n} (h : instRel P r x y) :
    instRel P r (x.corrupt P id) (y.corrupt P id) := by
  have hR : InstRel P x y := h
  exact
    { inv := hR.inv.step (ImplementationStep.fail (r := r) x id)
...
theorem implementation_refines (P : Parameters) (r : ℕ) :
  ...
  (ForwardSimulation.toProbabilistic (implementation_isLTS P r) (specInst_isLTS P r)
    (instRel_init P r) (refinesSpecification P r)).achievableTraceDists_subset
