...
gather-over-BRB-specification level (`ABA/GatherMid.lean`): the same fused
rule table as the GBCA-over-gather instance (`ABA/GBCAPair.lean`), each
...
abbrev IdealState (n : ℕ) : Type :=
  Gather.MidState n Bool × Gather.MidState n (Option Bool)

/-- The initial state. -/
def IdealState.initial (n : ℕ) : IdealState n :=
  (Gather.MidState.initial n Bool, Gather.MidState.initial n (Option Bool))
...
inductive IdealStep (P : Params) (r : ℕ) :
  ...
  | callG (s : IdealState P.n) (id : Fin P.n) (b : Bool)
      (t1' : Gather.MidState P.n Bool)
      (h : Gather.MidStep P s.1 (.call id b) (PMF.pure t1')) :
      IdealStep P r s (.callG r id b) (PMF.pure (t1', s.2))
  /-- An internal step of the first gather instance. -/
  | ga1Tau (s : IdealState P.n) (t1' : Gather.MidState P.n Bool)
      (h : Gather.MidStep P s.1 Gather.Lab.tau (PMF.pure t1')) :
      IdealStep P r s .tau (PMF.pure (t1', s.2))
  /-- An internal step of the second gather instance. -/
  | ga2Tau (s : IdealState P.n) (t2' : Gather.MidState P.n (Option Bool))
      (h : Gather.MidStep P s.2 Gather.Lab.tau (PMF.pure t2')) :
  ...
  | link (s : IdealState P.n) (id : Fin P.n) (g : Fin P.n → Option Bool)
      (t1' : Gather.MidState P.n Bool)
      (h : Gather.MidStep P s.1 (.ret id g) (PMF.pure t1'))
  ...
  | retG (s : IdealState P.n) (id : Fin P.n) (g : Fin P.n → Option (Option Bool))
      (t2' : Gather.MidState P.n (Option Bool))
      (h : Gather.MidStep P s.2 (.ret id g) (PMF.pure t2')) :
