...
`LowSubStep` is the rule table of `Gather.lowSub` (`ABA/Gather/Sub.lean`) — the
...
`lowSub_step_iff_row` is the row characterisation: at a specification label
`l₀`, the transitions of the composition over the labels `specPull` sends to
`l₀` are exactly the `l₀`-rows of `LowSubStep`, on the same state and with the
same distribution.

## The broadcast tier

A broadcast instance's own rows are `BRB.ImplStep` (`ABA/Broadcast/Impl.lean`),
and `BRB.sub_step_iff_row` matches them against the instance's transitions. Each
label of the composition reaches an instance at one label of its interface
alphabet, and the rows there carry over: a silent step, a return and a
...
theorem sub_step_at {M : Type} [DecidableEq M] {P : Params} {ldr : Fin P.n}
    {s s' : BRB.ImplState P.n M} {l : BRB.SubLab P.n M} {l₀ : BRB.Lab P.n M}
    (hl : BRB.specPull P.n M l = some l₀)
    (h : (BRB.sub P ldr M).step s l (PMF.pure s')) : BRB.ImplStep P ldr s l₀ (PMF.pure s') := by
  obtain ⟨l₁, hl₁, hrow⟩ := BRB.sub_step_row P ldr s l _ h
...
theorem row_sub_step_inl {M : Type} [DecidableEq M] {P : Params} {ldr : Fin P.n}
    {s s' : BRB.ImplState P.n M} {l₀ : BRB.Lab P.n M} (h0 : ∀ m : M, l₀ ≠ BRB.Lab.call m)
    (h : BRB.ImplStep P ldr s l₀ (PMF.pure s')) :
    (BRB.sub P ldr M).step s (Sum.inl l₀) (PMF.pure s') := by
  obtain ⟨l, hl, hstep⟩ := BRB.row_sub_step P ldr s l₀ _ h
  cases l with
...
theorem sub_call_row {M : Type} [DecidableEq M] {P : Params} {ldr : Fin P.n}
    {s s' : BRB.ImplState P.n M} {m : M}
    (h : (BRB.sub P ldr M).step s (Sum.inl (BRB.Lab.call m)) (PMF.pure s')) :
    (s.proc ldr).input = none ∧
      s' = (s.setProc ldr { s.proc ldr with input := some m }).mcast ldr (.init m) := by
  obtain ⟨u, w⟩ := s
  rcases (BRB.sub_step_iff P ldr (u, w) (Sum.inl (BRB.Lab.call m)) _).mp h with ⟨hτ, -⟩ | hlab
  · exact absurd hτ (by simp)
  · obtain ⟨x, w', hμ, hall, hn⟩ := BRB.subPre_joint_inv (by simp) hlab
    ...
    subst hw
    refine ⟨hinp, ?_⟩
    rw [PMF.pure_injective hμ, BRB.sub_setProc_post (PMF.pure_injective hx) hfor]
    rfl

omit [DecidableEq X] in
/-- At the call-loop label the instance stands still. -/
theorem sub_callLoop_row {M : Type} [DecidableEq M] {P : Params} {ldr : Fin P.n}
    {s s' : BRB.ImplState P.n M} {m : M}
    (h : (BRB.sub P ldr M).step s (Sum.inr (BRB.Extra.callLoop m)) (PMF.pure s')) : s' = s := by
  obtain ⟨u, w⟩ := s
  rcases (BRB.sub_step_iff P ldr (u, w) (Sum.inr (BRB.Extra.callLoop m)) _).mp h with
    ⟨hτ, -⟩ | hlab
  · exact absurd hτ (by simp)
  · obtain ⟨x, w', hμ, hall, hn⟩ := BRB.subPre_joint_inv (by simp) hlab
    ...
    rw [PMF.pure_injective hμ, BRB.sub_idle hidle]

omit [DecidableEq X] in
/-- Build the instance's broadcast at the call label. -/
theorem row_sub_call_step {M : Type} [DecidableEq M] (P : Params) (ldr : Fin P.n)
    (s : BRB.ImplState P.n M) (m : M) (h : (s.proc ldr).input = none) :
    (BRB.sub P ldr M).step s (Sum.inl (BRB.Lab.call m))
      (PMF.pure ((s.setProc ldr { s.proc ldr with input := some m }).mcast ldr (.init m))) := by
  obtain ⟨u, w⟩ := s
  exact BRB.sub_lab_step P ldr (by simp)
...
theorem row_sub_callLoop_step {M : Type} [DecidableEq M] (P : Params) (ldr : Fin P.n)
    (s : BRB.ImplState P.n M) (m : M) :
    (BRB.sub P ldr M).step s (Sum.inr (BRB.Extra.callLoop m)) (PMF.pure s) := by
  obtain ⟨u, w⟩ := s
  exact BRB.sub_lab_step P ldr (by simp) (fun i => BRB.ProcStep.callLoop (u i) m)
    (BRB.NetStep.callLoop w m)

/-! ### The rows -/

/-- The rows of the gather instance over Bracha's broadcast (`Gather.lowSub`),
stated over the composition's state: one constructor per case of
`Gather.lowSub_step_iff_row`. All transitions are Dirac. -/
inductive LowSubStep (P : Params) :
    LowSubState P.n X → Lab P.n X → PMF (LowSubState P.n X) → Prop
  /-- The call arrives: the gather record records the payload and the input
  instance broadcasts it. -/
  | call (s : LowSubState P.n X) (id : Fin P.n) (x : X)
      (h : ((ga s).proc id).input = none) (hb : ((brbIn s id).proc id).input = none) :
      LowSubStep P s (.call id x)
  ...
  | callLoop (s : LowSubState P.n X) (id : Fin P.n) (x : X) :
      LowSubStep P s (.call id x) (PMF.pure s)
  /-- A silent step of one input instance. -/
  | brbInTau (s : LowSubState P.n X) (k : Fin P.n) (c : BRB.ImplState P.n X)
      (hb : BRB.ImplStep P k (brbIn s k) .tau (PMF.pure c)) :
      LowSubStep P s .tau (PMF.pure (setBrbIn s (Function.update (brbIn s) k c)))
  /-- A silent step of one bind instance. -/
  | brbBindTau (s : LowSubState P.n X) (q : Fin P.n) (d : BRB.ImplState P.n (APSet P.n X))
      (hb : BRB.ImplStep P q (brbBind s q) .tau (PMF.pure d)) :
      LowSubStep P s .tau (PMF.pure (setBrbBind s (Function.update (brbBind s) q d)))
  /-- Asynchronous delivery on the gather network. -/
  | deliver (s : LowSubState P.n X) (i j : Fin P.n) (m : GaMsg P.n X)
      (h : m ∈ (ga s).sent j) :
      LowSubStep P s .tau (PMF.pure (setGa s ((ga s).recvMsg i j m)))
  /-- `ECHO`: the process holds every pair of a payload set of at least `n − f`
  pairs. -/
  | echo (s : LowSubState P.n X) (j : Fin P.n) (A : APSet P.n X)
  ...
  | vote (s : LowSubState P.n X) (j : Fin P.n) (U : APSet P.n X)
  ...
  bind payload, are delivered here, and the bind instance broadcasts it. -/
  | bindCall (s : LowSubState P.n X) (j : Fin P.n) (U : APSet P.n X)
      (hin : ((ga s).proc j).input ≠ none) (happ : approvedBy ((ga s).proc j) U)
      (hQ : ∃ Q : Finset (Fin P.n), P.n - P.f ≤ Q.card ∧
  ...
  | byz (s : LowSubState P.n X) (j : Fin P.n) (m : GaMsg P.n X) (h : j ∈ (ga s).F) :
      LowSubStep P s .tau (PMF.pure (setGa s ((ga s).mcast j m)))
  /-- An input instance returns to `j`, which files the value in its store. -/
  | inRet (s : LowSubState P.n X) (k j : Fin P.n) (v : X) (c : BRB.ImplState P.n X)
  ...
  | bindRet (s : LowSubState P.n X) (q j : Fin P.n) (U : APSet P.n X)
  ...
  /-- Return: the output's entries are held here, and `n − f` bind payloads held
  here are sub-maps of it. The label carries the instance's core, which this row
  writes if it is unwritten. -/
  | ret (s : LowSubState P.n X) (id : Fin P.n) (g : Fin P.n → Option X)
      ...
      LowSubStep P s (.ret id g ((core s).getD (coreOfNet P (ga s).2)))
  ...
  | fail (s : LowSubState P.n X) (id : Fin P.n) :
      LowSubStep P s (.fail id)
        (PMF.pure (corruptAll P id (SubState.corrupt P id) (SubState.corrupt P id) s))

/-! ### The row characterisation -/

/-- **The projection.** -/
theorem lowSub_step_row (P : Params) :
    ∀ (s : LowSubState P.n X) (l : SubLab P.n X) (μ : PMF (LowSubState P.n X)),
      (lowSub P X).step s l μ →
      ∃ l₀, specPull P.n X l = some l₀ ∧ LowSubStep P s l₀ μ := by
  have hIn : ∀ k : Fin P.n, (BRB.sub P k X).IsLTS := fun k => BRB.sub_isLTS P k
  have hBind : ∀ q : Fin P.n, (BRB.sub P q (APSet P.n X)).IsLTS := fun q => BRB.sub_isLTS P q
  rintro ⟨⟨u, w⟩, a, b⟩ l μ hstep
  rcases (subAt_step_iff P X (fun k => BRB.sub P k X)
      (fun q => BRB.sub P q (APSet P.n X)) _ l μ).mp hstep with ⟨rfl, e, hev⟩ | hlab
  · obtain ⟨x, w', a', b', rfl, hproc, hnet, hin, hbind⟩ :=
    ...
    | snd j m =>
      ...
      | echo A =>
        ...
        exact LowSubStep.echo _ j A hinp happ hcard hsend
      | vote U =>
        obtain ⟨hinp, happ, hQ, hsend, hx⟩ := stepG_snd_vote_own (hproc j)
        rw [sub_setProc_post (PMF.pure_injective hx) hfor]
        exact LowSubStep.vote _ j U hinp happ hQ hsend
    | dlv i j m =>
    ...
    | inRet k j v =>
      ...
      exact LowSubStep.inRet _ k j v (a' k) himpl
    | bindCall j U =>
    ...
    | bindRet q j U =>
      ...
      exact LowSubStep.bindRet _ q j U (b' q) himpl
  · by_cases hlτ : l = Sum.inl Lab.tau
    · subst hlτ
      ...
      · obtain ⟨jj, m, hF, hv⟩ := netStep_tau hn
        ...
        subst hv'
        rw [sub_post]
        exact LowSubStep.byz _ jj m hF
      · have himpl : BRB.ImplStep P k (a k) BRB.Lab.tau (PMF.pure c) :=
          sub_step_at (l := (Silent.τ : BRB.SubLab P.n X)) rfl hs
        rw [sub_setBrbIn]
        exact LowSubStep.brbInTau _ k c himpl
      · have himpl : BRB.ImplStep P q (b q) BRB.Lab.tau (PMF.pure d) :=
          sub_step_at (l := (Silent.τ : BRB.SubLab P.n (APSet P.n X))) rfl hs
        rw [sub_setBrbBind]
        exact LowSubStep.brbBindTau _ q d himpl
    · obtain ⟨x, w', a', b', rfl, hproc, hnet, hin, hbind⟩ :=
      ...
      | inl l₀ =>
        ...
        | call id y =>
          ...
          exact LowSubStep.call _ id y hinp hbin
        | ret id g C =>
          ...
          exact LowSubStep.ret _ id g hinp hsub hQ hr
        | fail id =>
          ...
          have ha : ∀ k, a' k = SubState.corrupt P id (a k) := fun k =>
            PMF.pure_injective (implStep_fail (sub_step_at (l := Sum.inl (BRB.Lab.fail id)) rfl
      ...
      | inr ev =>
        ...
        | callLoop id y =>
          ...
          have haid : a' id = a id := sub_callLoop_row
          ...
          refine ⟨Lab.call id y, rfl, ?_⟩
          rw [sub_idle hxall]
          exact LowSubStep.callLoop _ id y

/-- **The embedding.** -/
theorem row_lowSub_step (P : Params) :
    ∀ (s : LowSubState P.n X) (l₀ : Lab P.n X) (μ : PMF (LowSubState P.n X)),
      LowSubStep P s l₀ μ →
      ∃ l, specPull P.n X l = some l₀ ∧ (lowSub P X).step s l μ := by
  rintro ⟨⟨u, w⟩, a, b⟩ l₀ μ hrow
  cases hrow with
  | call id x h hb =>
    exact ⟨Sum.inl (.call id x), rfl, subAt_lab_step (b' := b) (by simp)
      (procStep_update (ProcStep.call (u id) x h)
        (fun i hi => ProcStep.callIdle (u i) id x (Ne.symm hi)))
      (NetStep.call w id x)
      (lift_update (by simp) (fun k hk => by simp [hk]) (row_sub_call_step P id (a id) x hb))
      (fun q => lift_idle rfl)⟩
  | callLoop id x =>
    ...
    · by_cases hk : k = id
      · subst hk; exact row_lift_step (by simp) (row_sub_callLoop_step P k (a k) x)
      · exact lift_idle (by simp [hk])
  | brbInTau k c hb =>
    exact ⟨Sum.inl Lab.tau, rfl, subAt_tau_in (row_sub_step_inl (by simp) hb)⟩
  | brbBindTau q d hb =>
    exact ⟨Sum.inl Lab.tau, rfl, subAt_tau_bind (row_sub_step_inl (by simp) hb)⟩
  | deliver i j m h =>
    ...
    refine ⟨Sum.inl Lab.tau, rfl,
      subAt_event_step (x := u) (w' := w) (a' := a) (GaEvt.bindCall j U) (fun i => ?_)
        ...
        (lift_update (by simp) (fun q hq => by simp [hq])
          (row_sub_call_step P j (b j) U hbc))⟩
    by_cases hi : i = j
  ...
  | inRet k j v c hb =>
    exact ⟨Sum.inl Lab.tau, rfl, subAt_event_step (w' := w) (b' := b) (GaEvt.inRet k j v)
      ...
      (lift_update (by simp) (fun k' hk' => by simp [hk']) (row_sub_step_inl (by simp) hb))
      (fun q => lift_idle rfl)⟩
  | bindRet q j U d hb =>
    exact ⟨Sum.inl Lab.tau, rfl, subAt_event_step (w' := w) (a' := a) (GaEvt.bindRet q j U)
      ...
      (lift_update (by simp) (fun q' hq' => by simp [hq']) (row_sub_step_inl (by simp) hb))⟩
  ...
  | fail id =>
    exact ⟨Sum.inl (.fail id), rfl, subAt_lab_step (x := u) (by simp)
      ...
      (lift_all (l₀ := fun _ => Sum.inl (BRB.Lab.fail id)) (fun k => rfl)
        (fun k => row_sub_step_inl (by simp) (BRB.ImplStep.fail (a k) id)))
      (lift_all (l₀ := fun _ => Sum.inl (BRB.Lab.fail id)) (fun q => rfl)
        (fun q => row_sub_step_inl (by simp) (BRB.ImplStep.fail (b q) id)))⟩
...
`l₀`-rows of `LowSubStep`, on the same state and with the same distribution. -/
theorem lowSub_step_iff_row (P : Params) (s : LowSubState P.n X) (l₀ : Lab P.n X)
    (μ : PMF (LowSubState P.n X)) :
    (∃ l, specPull P.n X l = some l₀ ∧ (lowSub P X).step s l μ) ↔ LowSubStep P s l₀ μ := by
  constructor
  · rintro ⟨l, hl, hstep⟩
    obtain ⟨l₁, hl₁, hrow⟩ := lowSub_step_row P s l μ hstep
    have hll : l₁ = l₀ := Option.some.inj (show (some l₁ : Option (Lab P.n X)) = some l₀ by
      rw [← hl₁, hl])
    subst hll
    exact hrow
  · exact row_lowSub_step P s l₀ μ

/-- info: 'PLTS.ABA.Gather.lowSub_step_iff_row' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms lowSub_step_iff_row
