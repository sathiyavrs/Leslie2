...
private theorem commitsIn_reach {s : LowState P.n X} {j : Fin P.n} :
    ∀ (l : List (Fin P.n × X)) (t : IdealState P.n X),
      LowRel P s t →
      (∀ p ∈ l, apIn P s j p.1 p.2) →
      ∃ ts : List (IdealState P.n X),
        List.IsChain (fun a b => IdealStep P a Gather.Lab.tau (PMF.pure b)) (t :: ts) ∧
  ...
  | p :: l, t, hR, hap => by
    ...
    · have hm' : p.1 ∈ t.ga.F ∨ (t.brbIn p.1).input = some p.2 := by
        ...
        · left
          ...
          exact hF
        · exact Or.inr hin
      have hstep : IdealStep P t Gather.Lab.tau (PMF.pure
          { t with
            brbIn := Function.update t.brbIn p.1 { t.brbIn p.1 with val := some p.2 } }) :=
        IdealStep.commitIn t p.1 p.2 hval hm'
      have hR1 : LowRel P s
          { t with
...
private theorem commitsBind_reach {s : LowState P.n X} {j : Fin P.n} :
    ∀ (l : List (Fin P.n × APSet P.n X)) (t : IdealState P.n X),
      LowRel P s t →
  ...
  | p :: l, t, hR, hap => by
    ...
    · have hm' : p.1 ∈ t.ga.F ∨ (t.brbBind p.1).input = some p.2 := by
      ...
      have hstep : IdealStep P t Gather.Lab.tau (PMF.pure
        ...
        IdealStep.commitBind t p.1 p.2 hval hm'
      have hR1 : LowRel P s
          { t with
...
theorem lowRel_call {s : LowState P.n X} {t : IdealState P.n X}
...
theorem lowTau_reach {s s' : LowState P.n X} {t : IdealState P.n X}
  ...
  | vote j U hin happ hQ hsend =>
    ...
    have hga1 : (ts.getLastD t).ga = s.ga := by
      rw [hga, hR.ga_eq]
    have hvotestep : IdealStep P (ts.getLastD t) Gather.Lab.tau
        (PMF.pure { ts.getLastD t with
          ga := ((ts.getLastD t).ga.setProc j
            { (ts.getLastD t).ga.proc j with sentVote := some U }).mcast j
            (.vote U) }) := by
      refine IdealStep.vote _ j U ?_ ?_ ?_ ?_
      · rw [hga1]
        exact hin
      · intro p hp
        exact hcov p (Finset.mem_toList.mpr hp)
  ...
  | bindCall j U hin hbc happ hQ =>
    ...
    have hga1 : (ts.getLastD t).ga = s.ga := by
      rw [hga, hR.ga_eq]
    have hbindstep : IdealStep P (ts.getLastD t) Gather.Lab.tau
        (PMF.pure { ts.getLastD t with
          brbBind := Function.update (ts.getLastD t).brbBind j
            { (ts.getLastD t).brbBind j with input := some U } }) := by
      refine IdealStep.bindCall _ j U ?_ ?_ ?_ ?_
      · rw [hga1]
        exact hin
...
theorem gatherLow (P : Params) (X : Type) [DecidableEq X] :
    ForwardSimulation (lowInst P X) (idealInst P X) (LowRel P) := by
  constructor
  ...
  | call id x h hb =>
    ...
    exact ⟨_, Or.inr ⟨by simp, System.weakLStep_of_step (by simp)
      (IdealStep.call q₂ id x (by rw [hR.ga_eq]; exact h))⟩,
      lowRel_call hR h hb⟩
  | callLoop id x =>
    rw [PMF.mem_support_pure_iff] at hq₁'
    subst hq₁'
    exact ⟨q₂, Or.inr ⟨by simp, System.weakLStep_of_step (by simp)
      (IdealStep.callLoop q₂ id x)⟩, hR⟩
  | brbInTau k b' h =>
    rw [PMF.mem_support_pure_iff] at hq₁'
    subst hq₁'
  ...
  | fail id =>
    ...
    exact ⟨q₂.corruptAll P id, Or.inr ⟨by simp, System.weakLStep_of_step (by simp)
      (IdealStep.fail q₂ id)⟩, lowRel_corrupt hR id⟩
