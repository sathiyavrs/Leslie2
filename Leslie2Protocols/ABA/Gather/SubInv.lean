...
The invariant of `Gather.idealSub` (`ABA/Gather/Sub.lean`), stated over the
...
`ABA/Gather/Core.lean`, applied to `netOf`.
...
def netOf {n : ℕ} (s : IdealSubState n X) : SubState n (PRec n X) (GaMsg n X) :=
  ((fun _ => LocalState.initial n (GaMsg n X) (PRec.initial n X)), (ga s).2)

omit [DecidableEq X] in
@[simp] theorem netOf_sent {n : ℕ} (s : IdealSubState n X) :
    (netOf s).sent = (ga s).sent := rfl

omit [DecidableEq X] in
@[simp] theorem netOf_F {n : ℕ} (s : IdealSubState n X) : (netOf s).F = (ga s).F := rfl

omit [DecidableEq X] in
/-- The core of the composition's gather network state. -/
theorem coreOf_netOf (s : IdealSubState P.n X) :
...
per instance. -/
structure IdealConf (P : Params) (s : IdealSubState P.n X) : Prop where
...
theorem IdealConf.initial : IdealConf P ((idealSub P X).init) := by
...
theorem IdealConf.step {s : IdealSubState P.n X} {l : Lab P.n X}
    {μ : PMF (IdealSubState P.n X)} (hInv : IdealConf P s) (hstep : IdealSubStep P s l μ)
    {s' : IdealSubState P.n X} (hs' : s' ∈ μ.support) : IdealConf P s' := by
  cases hstep with
  | call id x h hb =>
...
returned it. -/
theorem approved_of_approvedBy {s : IdealSubState P.n X} (hConf : IdealConf P s)
...
every rule. -/
theorem approved_mono {s s' : IdealSubState P.n X} {l : Lab P.n X}
    {μ : PMF (IdealSubState P.n X)} (hstep : IdealSubStep P s l μ)
    (hs' : s' ∈ μ.support) {A : APSet P.n X} (h : approved s A) : approved s' A := by
  have key : ∀ t : IdealSubState P.n X,
...
theorem echoAppr_initial :
    ∀ (j : Fin P.n) (A : APSet P.n X),
      ((ga ((idealSub P X).init)).proc j).sentEcho = some A →
        approved ((idealSub P X).init) A := by
  intro j A hA
  simp [ga, SubState.proc, ProcRec.initial, PRec.initial] at hA

/-- **The approval of an `ECHO` field is inductive**: only `IdealSubStep.echo`
writes the field, and its guard is the approval of the payload it writes by the
writer's own store. -/
theorem echoAppr_step {s s' : IdealSubState P.n X} {l : Lab P.n X}
    {μ : PMF (IdealSubState P.n X)} (hConf : IdealConf P s)
    (hEA : ∀ (j : Fin P.n) (A : APSet P.n X),
      ((ga s).proc j).sentEcho = some A → approved s A)
    (hstep : IdealSubStep P s l μ) (hs' : s' ∈ μ.support) :
    ∀ (j : Fin P.n) (A : APSet P.n X),
...
structure IdealInv (P : Params) (s : IdealSubState P.n X) : Prop extends IdealConf P s where
  /-- The payload set in a process's `ECHO` field consists of committed input
  entries. No honesty side condition: only `IdealSubStep.echo` writes the field,
  and its guard holds of a corrupted sender too. -/
  echo_appr : ∀ (j : Fin P.n) (A : APSet P.n X),
    ((ga s).proc j).sentEcho = some A → approved s A

/-- The invariant holds initially. -/
theorem IdealInv.initial : IdealInv P ((idealSub P X).init) :=
  ⟨IdealConf.initial, echoAppr_initial⟩

/-- The invariant is preserved by every step. -/
theorem IdealInv.step {s : IdealSubState P.n X} {l : Lab P.n X}
    {μ : PMF (IdealSubState P.n X)} (hInv : IdealInv P s) (hstep : IdealSubStep P s l μ)
    {s' : IdealSubState P.n X} (hs' : s' ∈ μ.support) : IdealInv P s' :=
...
holds. -/
theorem echoOf_eq {s : IdealSubState P.n X} (hInv : IdealInv P s) {j : Fin P.n}
...
everything. -/
theorem dominatedBy_card {s : IdealSubState P.n X} (hInv : IdealInv P s) {q : Fin P.n}
...
`n − f − |F|` of them. -/
theorem dominatedBy_honest_card {s : IdealSubState P.n X} (hInv : IdealInv P s)
...
dominators. -/
theorem exists_dominators {s : IdealSubState P.n X} (hInv : IdealInv P s) :
    ∃ j₀, j₀ ∈ honest (netOf s) ∧
...
theorem transfer {s : IdealSubState P.n X} (hInv : IdealInv P s) {j₀ : Fin P.n}
...
`BIND` payload. -/
theorem core_witness {s : IdealSubState P.n X} (hInv : IdealInv P s)
    {k₀ : Fin P.n} (hk₀ : k₀ ∉ (ga s).F) {U₀ : APSet P.n X}
    (hU₀ : (brbBind s k₀).val = some U₀) :
...
theorem single_core {s : IdealSubState P.n X} (hInv : IdealInv P s)
    {k₀ : Fin P.n} (hk₀ : k₀ ∉ (ga s).F) {U₀ : APSet P.n X}
    (hU₀ : (brbBind s k₀).val = some U₀) :
...
theorem single_core_approved {s : IdealSubState P.n X} (hInv : IdealInv P s)
    {k₀ : Fin P.n} (hk₀ : k₀ ∉ (ga s).F) {U₀ : APSet P.n X}
    (hU₀ : (brbBind s k₀).val = some U₀) :
...
is blind to `F`. -/
noncomputable def bindAbove (s : IdealSubState P.n X) (C : APSet P.n X) :
    Finset (Fin P.n) :=
  Finset.univ.filter (fun q => ∃ U, (brbBind s q).val = some U ∧ C ⊆ U)

open scoped Classical in
theorem mem_bindAbove {s : IdealSubState P.n X} {C : APSet P.n X} {q : Fin P.n} :
...
theorem bindVal_mono {s s' : IdealSubState P.n X} {l : Lab P.n X}
    {μ : PMF (IdealSubState P.n X)} (hstep : IdealSubStep P s l μ) (hs' : s' ∈ μ.support)
...
theorem bindAbove_mono {s s' : IdealSubState P.n X} {l : Lab P.n X}
    {μ : PMF (IdealSubState P.n X)} (hstep : IdealSubStep P s l μ) (hs' : s' ∈ μ.support)
...
theorem coreOf_freeze {s : IdealSubState P.n X} (hInv : IdealInv P s)
