...
`Gather.Sub.gatherCore`: the composed gather instance over broadcast
specifications (`ABA/Gather/Sub.lean`) forward-simulates the gather
specification read over the instance's interface, along `Gather.Sub.CoreRel`.

A transition of the instance is one row of `IdealSubStep`
(`Gather.idealSub_step_row`), the row is answered by a weak run of the gather
...
* The core frozen is `coreOfNet` of the instance's gather network state, and the
  two guards of `bindCore` are `Gather.Sub.coreOf_freeze`, which the returner's
...
* Every return, the first included, is matched through the count
  ...
  payloads are written once (`Gather.Sub.bindAbove_mono`) — so it survives every
...
the relabelled trace, and `idealSub_core` transfers it along the refinement.
-/
...
structure CoreRel (P : Params) (s : IdealSubState P.n X) (t : SpecState P.n X) : Prop where
...
theorem coreRel_init :
    CoreRel P ((idealSub P X).init) ((liftedSpec P X).init) := by
...
sides at once. -/
theorem coreRel_corrupt {s : IdealSubState P.n X} {t : SpecState P.n X}
    (hR : CoreRel P s t) (id : Fin P.n) :
    CoreRel P (corruptAll P id (BRB.SpecState.corrupt P id) (BRB.SpecState.corrupt P id) s)
      (t.corrupt P id) := by
  refine ⟨hR.inv.step (IdealSubStep.fail s id) (by rw [PMF.mem_support_pure_iff]),
    ?_, ?_, ?_, ?_, ?_, ?_⟩
  ...
  · intro C hC
    exact le_trans (hR.core_cert C hC) (Finset.card_le_card
      (bindAbove_mono (IdealSubStep.fail s id) (by rw [PMF.mem_support_pure_iff]) C))
...
theorem retRun {s : IdealSubState P.n X} {t : SpecState P.n X}
  ...
  have hInv' := hR.inv.step (IdealSubStep.ret s id g hin hsub hQ hr)
...
theorem coreRel_call {s : IdealSubState P.n X} {t : SpecState P.n X}
  ...
  refine ⟨hR.inv.step (IdealSubStep.call s id x h hb) (by rw [PMF.mem_support_pure_iff]),
    ?_, ?_, ?_, ?_, ?_, ?_⟩
...
theorem coreRel_tau {s s' : IdealSubState P.n X} {t : SpecState P.n X}
    (hR : CoreRel P s t) (hstep : IdealSubStep P s Gather.Lab.tau (PMF.pure s')) :
    CoreRel P s' t := by
  have hInv' := hR.inv.step hstep (by rw [PMF.mem_support_pure_iff])
  generalize hμ : (PMF.pure s' : PMF (IdealSubState P.n X)) = μ at hstep
  cases hstep with
  | commitIn k v hv hm =>
  ...
  | commitBind q U hv hm =>
    ...
    intro C hC
    exact le_trans (hR.core_cert C hC) (Finset.card_le_card
      (bindAbove_mono (IdealSubStep.commitBind s q U hv hm)
  ...
  | bindCall j U hin happ hQ hb =>
    ...
    intro C hC
    exact le_trans (hR.core_cert C hC) (Finset.card_le_card
      (bindAbove_mono (IdealSubStep.bindCall s j U hin happ hQ hb)
        (by rw [PMF.mem_support_pure_iff]) C))
  ...
  | bindRet q j U hv hr =>
    ...
    · intro C hC
      exact le_trans (hR.core_cert C hC) (Finset.card_le_card
        (bindAbove_mono (IdealSubStep.bindRet s q j U hv hr)
          (by rw [PMF.mem_support_pure_iff]) C))


/-! ### The relation across one row -/

/-- **The relation across one row**: every row of `IdealSubStep` at a related
...
`commit* ; bindCore? ; ret`. -/
theorem coreRel_row (P : Params) (X : Type) [DecidableEq X] (q₁ : IdealSubState P.n X)
    (q₂ : SpecState P.n X) (hR : CoreRel P q₁ q₂) (l₀ : Lab P.n X)
    (μ : PMF (IdealSubState P.n X)) (hrow : IdealSubStep P q₁ l₀ μ)
    (q₁' : IdealSubState P.n X) (hq₁' : q₁' ∈ μ.support) :
  ...
  | callSpecLoop id x h =>
    ...
    refine ⟨hR.inv.step (IdealSubStep.callSpecLoop q₁ id x h)
      (by rw [PMF.mem_support_pure_iff]),
  ...
  | callProcLoop id x hb =>
    ...
    refine ⟨hR.inv.step (IdealSubStep.callProcLoop q₁ id x hb)
      (by rw [PMF.mem_support_pure_iff]),
  ...
  | commitIn k v hv hm =>
    ...
    exact ⟨q₂, Or.inl ⟨rfl, System.weakLSilent_refl _ q₂⟩,
      coreRel_tau hR (IdealSubStep.commitIn q₁ k v hv hm)⟩
  | commitBind q U hv hm =>
    rw [PMF.mem_support_pure_iff] at hq₁'
    subst hq₁'
    exact ⟨q₂, Or.inl ⟨rfl, System.weakLSilent_refl _ q₂⟩,
      coreRel_tau hR (IdealSubStep.commitBind q₁ q U hv hm)⟩
  | deliver i j m h =>
    rw [PMF.mem_support_pure_iff] at hq₁'
    subst hq₁'
    exact ⟨q₂, Or.inl ⟨rfl, System.weakLSilent_refl _ q₂⟩,
      coreRel_tau hR (IdealSubStep.deliver q₁ i j m h)⟩
  | echo j A hin happ hcard hsend =>
    rw [PMF.mem_support_pure_iff] at hq₁'
    subst hq₁'
    exact ⟨q₂, Or.inl ⟨rfl, System.weakLSilent_refl _ q₂⟩,
      coreRel_tau hR (IdealSubStep.echo q₁ j A hin happ hcard hsend)⟩
  | vote j U hin happ hQ hsend =>
    rw [PMF.mem_support_pure_iff] at hq₁'
    subst hq₁'
    exact ⟨q₂, Or.inl ⟨rfl, System.weakLSilent_refl _ q₂⟩,
      coreRel_tau hR (IdealSubStep.vote q₁ j U hin happ hQ hsend)⟩
  | bindCall j U hin happ hQ hb =>
    rw [PMF.mem_support_pure_iff] at hq₁'
    subst hq₁'
    exact ⟨q₂, Or.inl ⟨rfl, System.weakLSilent_refl _ q₂⟩,
      coreRel_tau hR (IdealSubStep.bindCall q₁ j U hin happ hQ hb)⟩
  | bindCallSpecLoop j U hin happ hQ =>
    rw [PMF.mem_support_pure_iff] at hq₁'
    subst hq₁'
  ...
  | byz j m h =>
    ...
    subst hq₁'
    exact ⟨q₂, Or.inl ⟨rfl, System.weakLSilent_refl _ q₂⟩,
      coreRel_tau hR (IdealSubStep.byz q₁ j m h)⟩
  | inRet k j v hv hr =>
    rw [PMF.mem_support_pure_iff] at hq₁'
    subst hq₁'
    exact ⟨q₂, Or.inl ⟨rfl, System.weakLSilent_refl _ q₂⟩,
      coreRel_tau hR (IdealSubStep.inRet q₁ k j v hv hr)⟩
  | bindRet q j U hv hr =>
    rw [PMF.mem_support_pure_iff] at hq₁'
    subst hq₁'
    exact ⟨q₂, Or.inl ⟨rfl, System.weakLSilent_refl _ q₂⟩,
      coreRel_tau hR (IdealSubStep.bindRet q₁ q j U hv hr)⟩
...
`IdealSubStep` (`Gather.idealSub_step_row`), the row is answered by a weak run
...
theorem gatherCore (P : Params) (X : Type) [DecidableEq X] :
    ForwardSimulation (idealSub P X) (liftedSpec P X) (CoreRel P) := by
  constructor
  intro q₁ q₂ hR l μ hstep q₁' hq₁'
  obtain ⟨l₀, hpull, hrow⟩ := idealSub_step_row P q₁ l μ hstep
  ...
  · refine Or.inr ⟨?_, weakLStep_liftedSpec P hτ hpull hweak⟩
    intro hl
    refine hτ ?_
    have h2 : specPull P.n X (Silent.τ : SubLab P.n X) = some l₀ := by rw [← hl]; exact hpull
...
theorem liftedSpec_step_down (P : Params) {s : SpecState P.n X} {l : SubLab P.n X}
...
`gatherCore`. -/
theorem idealSub_refines (P : Params) (X : Type) [DecidableEq X] :
    achievableTraceDists (idealSub P X) ⊆ achievableTraceDists (liftedSpec P X) :=
  (ForwardSimulation.toProbabilistic (idealSub_isLTS P) (liftedSpec_isLTS P)
    coreRel_init (gatherCore P X)).achievableTraceDists_subset

/-- **The composed gather instance binds one core.** -/
theorem idealSub_core (P : Params) (X : Type) [DecidableEq X] :
    ∀ D ∈ achievableTraceDists (idealSub P X), ∀ t, D t ≠ 0 →
      CoreTrace P (t.map subDown) :=
  safety_transfer (idealSub_refines P X) (liftedSpec_core P X)

/-! ### Mechanical axiom check -/

/-- info: 'PLTS.ABA.Gather.Sub.gatherCore' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms gatherCore

/-- info: 'PLTS.ABA.Gather.Sub.single_core' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms single_core

/-- info: 'PLTS.ABA.Gather.Sub.liftedSpec_core' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms liftedSpec_core

/-- info: 'PLTS.ABA.Gather.Sub.idealSub_core' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms idealSub_core
