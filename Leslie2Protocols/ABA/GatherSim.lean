...
(`ABA/GatherMid.lean`) forward-simulates the gather specification
(`ABA/GatherSpec.lean`), along `Gather.CoreRel`.
...
structure MidInv (P : Params) (s : MidState P.n X) : Prop where
...
theorem MidInv.initial : MidInv P (MidState.initial P.n X) := by
  refine ⟨?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_⟩ <;>
    simp [MidState.initial, PRec.initial, BRB.SpecState.initial]

/-- The invariant is preserved by every step. -/
theorem MidInv.step {s : MidState P.n X} {l : Lab P.n X}
    {μ : PMF (MidState P.n X)} (hInv : MidInv P s) (hstep : MidStep P s l μ)
    {s' : MidState P.n X} (hs' : s' ∈ μ.support) : MidInv P s' := by
  cases hstep with
  | call id x h =>
  ...
  | fail id =>
    ...
    · intro i k x hx
      ...
      rw [show (s.corruptAll P id).ga.sent = s.ga.sent from
        MidState.corruptAll_ga_sent P id s]
      exact hInv.recv_sub i k hx
    · intro k
      rw [MidState.corruptAll_brbIn_input, MidState.corruptAll_ga_proc]
      exact hInv.input_eq k
    · intro k v hv
      rw [MidState.corruptAll_brbIn_val] at hv
      rw [MidState.corruptAll_brbIn_input]
      rcases hInv.inVal_prov k v hv with hkF | hin
      · exact Or.inl (SubState.corrupt_F_subset s.ga id hkF)
      · exact Or.inr hin
    · intro k U hU
      rw [MidState.corruptAll_brbBind_val] at hU
      rw [MidState.corruptAll_brbBind_input]
      rcases hInv.bindVal_prov k U hU with hkF | hin
      · exact Or.inl (SubState.corrupt_F_subset s.ga id hkF)
      · exact Or.inr hin
    · intro j hj A hA
      rw [show (s.corruptAll P id).ga.sent = s.ga.sent from
        MidState.corruptAll_ga_sent P id s] at hA
      rw [MidState.corruptAll_ga_proc]
      exact hInv.echo_conf j (hF j hj) A hA
    · intro j hj A hA
      rw [MidState.corruptAll_ga_proc] at hA
      exact hInv.echo_card j (hF j hj) A hA
    · intro j hj W hW
      rw [show (s.corruptAll P id).ga.sent = s.ga.sent from
        MidState.corruptAll_ga_sent P id s] at hW
      rw [MidState.corruptAll_ga_proc]
      exact hInv.vote_conf j (hF j hj) W hW
    · intro j hj W hW
      rw [MidState.corruptAll_ga_proc] at hW
      rw [show (s.corruptAll P id).ga.recv = s.ga.recv from rfl]
      exact hInv.vote_backed j (hF j hj) W hW
    · intro j hj U hU
      rw [MidState.corruptAll_brbBind_input] at hU
      rw [show (s.corruptAll P id).ga.recv = s.ga.recv from rfl]
...
entries it contains. -/
theorem vote_card {s : MidState P.n X} (hInv : MidInv P s) {j : Fin P.n}
...
both. -/
theorem bindInput_inter {s : MidState P.n X} (hInv : MidInv P s)
...
theorem bindVal_inter {s : MidState P.n X} (hInv : MidInv P s)
    {j j' : Fin P.n} (hj : j ∉ s.ga.F) (hj' : j' ∉ s.ga.F)
...
structure CoreRel (P : Params) (s : MidState P.n X) (t : SpecState P.n X) : Prop where
...
theorem coreRel_init :
    CoreRel P (MidState.initial P.n X) (SpecState.initial P.n X) := by
  refine ⟨MidInv.initial, ?_, ?_, ?_, ?_, ?_⟩ <;>
    simp [MidState.initial, SpecState.initial, PRec.initial]

/-- **Broadcast compatibility**: the relation is preserved by corrupting both
sides at once. -/
theorem coreRel_corrupt {s : MidState P.n X} {t : SpecState P.n X}
    (hR : CoreRel P s t) (id : Fin P.n) :
    CoreRel P (s.corruptAll P id) (t.corrupt P id) := by
  refine ⟨hR.inv.step (MidStep.fail s id) (by rw [PMF.mem_support_pure_iff]),
    ?_, ?_, ?_, ?_, ?_⟩
  · intro k
    rw [corrupt_call, MidState.corruptAll_ga_proc]
    exact hR.call_eq k
  · intro k
    rw [corrupt_ret, MidState.corruptAll_ga_proc]
    exact hR.ret_eq k
  · show (t.corrupt P id).F = (s.ga.corrupt P id).F
    rw [SpecState.corrupt_F, SubState.corrupt_F, hR.F_eq]
  · intro k v hv
    rw [corrupt_val] at hv
    rw [MidState.corruptAll_brbIn_val]
  ...
  · intro Cs hCs
    ...
    obtain ⟨-, U, hU, hval⟩ := hq
    exact ⟨Finset.mem_univ q, U, hU, by rw [MidState.corruptAll_brbBind_val]; exact hval⟩
...
theorem gatherCore (P : Params) (X : Type) [DecidableEq X] :
    ForwardSimulation (midInst P X) (specInst P X) (CoreRel P) := by
  constructor
  intro q₁ q₂ hR l μ hstep q₁' hq₁'
  rw [midInst_step] at hstep
