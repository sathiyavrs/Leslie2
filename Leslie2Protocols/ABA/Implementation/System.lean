...
def writeGhost (s : NetStateP n M G)
    (ghostStep : ExtendedLabel n M → NetStateP n M G → G → G) (L : ExtendedLabel n M) :
    NetStateP n M G :=
...
inductive ProgramStep (P : Params) (M S : Type)
    (roundStep : Fin P.n → ProcRecP P.n S → ExtendedLabel P.n M →
      PMF (ProcRecP P.n S) → Prop) (j : Fin P.n) :
    ProcRecP P.n S → ExtendedLabel P.n M → PMF (ProcRecP P.n S) → Prop
  /-- A row of the graded-agreement implementation. -/
  | stageRow (q : ProcRecP P.n S) (L : ExtendedLabel P.n M) (μ : PMF (ProcRecP P.n S))
  ...
  | corruptedIdle (c : CoreRec P.n) (p : StageSideRecP S) (L : ExtendedLabel P.n M)
  ...
  | retWPub (c : CoreRec P.n) (p : StageSideRecP S)
      ...
      ProgramStep P M S roundStep j (c, p) (Sum.inr (.retWPub r j co b))
        (PMF.pure (c.stepRound co, p))
  ...
  | retWPubIdle (c : CoreRec P.n) (p : StageSideRecP S)
      ...
      ProgramStep P M S roundStep j (c, p) (Sum.inr (.retWPub r id co b))
        (PMF.pure (c, p))
  /-- Such a call at another process: not `j`'s business. -/
  | gcallLoopIdle (c : CoreRec P.n) (p : StageSideRecP S)
      (r : ℕ) (id : Fin P.n) (b : Bool) (hid : id ≠ j) :
      ProgramStep P M S roundStep j (c, p) (Sum.inr (.gcallLoop r id b))
        (PMF.pure (c, p))
  ...
  | byzantineCallGIdle (c : CoreRec P.n) (p : StageSideRecP S)
  ...
  | byzantineCallGLoopIdle (c : CoreRec P.n) (p : StageSideRecP S)
  ...
  business. The process the label names has no row either (D11, D22). -/
  | byzantineRetGIdle (c : CoreRec P.n) (p : StageSideRecP S)
  ...
  no process moves. -/
  | byzantineCallWIdle (c : CoreRec P.n) (p : StageSideRecP S)
      (r : ℕ) (k : Fin P.n) :
  ...
  | byzantineRetWIdle (c : CoreRec P.n) (p : StageSideRecP S)
      (r : ℕ) (k : Fin P.n) (b : Bool) :
...
outcome. -/
inductive NetworkStep (P : Params) (M G : Type) [DecidableEq M]
    ...
    NetStateP P.n M G → ExtendedLabel P.n M → PMF (NetStateP P.n M G) → Prop
  ...
  yet (D12′). -/
  | decidedSend (s : NetStateP P.n M G) (j : Fin P.n) (b : Bool) (h : b ∉ s.dsent j) :
      NetworkStep P M G callPayload ghostStep ghostOut s (Sum.inr (.decidedSend j b))
        (PMF.pure ((s.dput j b).writeGhost ghostStep (Sum.inr (.decidedSend j b))))
  /-- The network's half of a DECIDED delivery: the payload must be sent
  under the named sender (D12′). -/
  | decidedDeliver (s : NetStateP P.n M G) (i j : Fin P.n) (b : Bool) (h : b ∈ s.dsent j) :
  ...
  | retWPub (s : NetStateP P.n M G) (r : ℕ) (id : Fin P.n) (c : Bool) (b : Bool) :
      NetworkStep P M G callPayload ghostStep ghostOut s (Sum.inr (.retWPub r id c b))
  ...
  | gcallLoop (s : NetStateP P.n M G) (r : ℕ) (id : Fin P.n) (b : Bool) :
      NetworkStep P M G callPayload ghostStep ghostOut s (Sum.inr (.gcallLoop r id b))
  ...
  | byzantineCallG (s : NetStateP P.n M G) (r : ℕ) (k : Fin P.n) (b : Bool) (hF : k ∈ s.F) :
  ...
  /-- A Byzantine graded-agreement call against an already-called stage record
  (D11). -/
  | byzantineCallGLoop (s : NetStateP P.n M G) (r : ℕ) (k : Fin P.n) (b : Bool)
      (hF : k ∈ s.F) :
  ...
  | byzantineRetG (s : NetStateP P.n M G) (r : ℕ) (k : Fin P.n) (out : GbcaOut) (bnd : Bool)
  ...
  | byzantineCallW (s : NetStateP P.n M G) (r : ℕ) (k : Fin P.n) (hF : k ∈ s.F) :
  ...
  | byzantineRetW (s : NetStateP P.n M G) (r : ℕ) (k : Fin P.n) (b : Bool) (hF : k ∈ s.F) :
  ...
  a corrupted sender. -/
  | byzantineG (s : NetStateP P.n M G) (r : ℕ) (k : Fin P.n) (m : M) (hF : k ∈ s.F) :
  ...
  | byzantineD (s : NetStateP P.n M G) (k : Fin P.n) (b : Bool) (hF : k ∈ s.F) :
      NetworkStep P M G callPayload ghostStep ghostOut s (Sum.inl .tau)
...
noncomputable def program (j : Fin P.n) : System (ProcRecP P.n S) (ExtendedLabel P.n M) where
...
@[simp] theorem program_step (j : Fin P.n) (q : ProcRecP P.n S)
    (l : ExtendedLabel P.n M) (μ : PMF (ProcRecP P.n S)) :
...
the coin oracle. -/
abbrev State (P : Params) (M S G : Type) : Type :=
...
/-- The network adversary. -/
noncomputable def network : System (NetStateP P.n M G) (ExtendedLabel P.n M) where
...
@[simp] theorem network_step (s : NetStateP P.n M G) (l : ExtendedLabel P.n M)
...
free (D29). -/
class IsRoundRuleTable (P : Params) (M S : Type)
    (roundStep : Fin P.n → ProcRecP P.n S → ExtendedLabel P.n M →
      PMF (ProcRecP P.n S) → Prop) : Prop where
  /-- A stage-side row carries a stage-side label. -/
  own : ∀ {j : Fin P.n} {q : ProcRecP P.n S} {L : ExtendedLabel P.n M}
    {μ : PMF (ProcRecP P.n S)}, roundStep j q L μ → roundOwn j L
  /-- A stage-side row fires only at an unreplaced program. -/
  honest : ∀ {j : Fin P.n} {q : ProcRecP P.n S} {L : ExtendedLabel P.n M}
    {μ : PMF (ProcRecP P.n S)}, roundStep j q L μ → q.1.corrupted = false
  /-- A stage-side row is Dirac. -/
  dirac : ∀ {j : Fin P.n} {q : ProcRecP P.n S} {L : ExtendedLabel P.n M}
    {μ : PMF (ProcRecP P.n S)}, roundStep j q L μ → ∃ q', μ = PMF.pure q'
...
theorem procStepN_dirac [IsRoundRuleTable P M S roundStep]
    {l : ExtendedLabel P.n M} (h : ProgramStep P M S roundStep j q l ν) :
    ∃ q', ν = PMF.pure q' := by
  cases h
  case stageRow h' => exact IsRoundRuleTable.dirac h'
...
theorem stepN_tau_terminate
  ...
  rw [nlab_tau] at h
  cases h
  case stageRow h' => exact (IsRoundRuleTable.own h').elim
...
theorem stepN_callG_foreign {r : ℕ} {id : Fin P.n} {b : Bool} (hid : id ≠ j)
    ...
    ν = PMF.pure q := by
  cases h
  case stageRow h' => exact absurd (IsRoundRuleTable.own h') hid
  case callGIdle => rfl
  case corruptedIdle => rfl
...
theorem stepN_gbcaSend_foreign {r : ℕ} {k : Fin P.n} {m : M} (hk : k ≠ j)
    (h : ProgramStep P M S roundStep j q (Sum.inr (.gbcaSend r k m)) ν) :
    ν = PMF.pure q := by
  cases h
  case stageRow h' => exact absurd (IsRoundRuleTable.own h') hk
  case gsndIdle => rfl
  case corruptedIdle => rfl

theorem stepN_gbcaDeliver_foreign {r : ℕ} {i k : Fin P.n} {m : M} (hi : i ≠ j)
    (h : ProgramStep P M S roundStep j q (Sum.inr (.gbcaDeliver r i k m)) ν) :
    ν = PMF.pure q := by
  cases h
  case stageRow h' => exact absurd (IsRoundRuleTable.own h') hi
  case gdlvIdle => rfl
  case corruptedIdle => rfl

theorem stepN_decidedSend_self {b : Bool}
...
theorem stepN_decidedSend_foreign {k : Fin P.n} {b : Bool} (hk : k ≠ j)
...
theorem stepN_decidedDeliver_self {k : Fin P.n} {b : Bool}
...
theorem stepN_decidedDeliver_foreign {i k : Fin P.n} {b : Bool} (hi : i ≠ j)
...
theorem stepN_retWPub_self {r : ℕ} {co b : Bool}
    (h : ProgramStep P M S roundStep j q (Sum.inr (.retWPub r j co b)) ν) :
    q.1.corrupted = false ∧
...
theorem stepN_retWPub_foreign {r : ℕ} {id : Fin P.n} {co b : Bool} (hid : id ≠ j)
    (h : ProgramStep P M S roundStep j q (Sum.inr (.retWPub r id co b)) ν) :
    ν = PMF.pure q := by
  cases h
  case stageRow h' => exact (IsRoundRuleTable.own h').elim
...
theorem stepN_gcallLoop_foreign {r : ℕ} {id : Fin P.n} {b : Bool} (hid : id ≠ j)
    (h : ProgramStep P M S roundStep j q (Sum.inr (.gcallLoop r id b)) ν) :
    ν = PMF.pure q := by
  cases h
  case stageRow h' => exact absurd (IsRoundRuleTable.own h') hid
...
theorem stepN_byzantineCallGLoop {r : ℕ} {k : Fin P.n} {b : Bool}
...
theorem stepN_byzantineCallW {r : ℕ} {k : Fin P.n}
...
theorem stepN_byzantineRetW {r : ℕ} {k : Fin P.n} {b : Bool}
...
theorem stepN_byzantineCallG_noStep {r : ℕ} {b : Bool}
...
(D11, D22, D23). -/
theorem stepN_byzantineRetG_noStep {r : ℕ} {out : GbcaOut} {bnd : Bool}
...
`IsRoundRuleTable.honest`, so no row of a replaced program survives except a
self-loop. -/
theorem stepN_inert {L : ExtendedLabel P.n M} (hc : q.1.corrupted = true)
    (h : ProgramStep P M S roundStep j q L ν) : ν = PMF.pure q := by
  cases h
  case stageRow h' => rw [IsRoundRuleTable.honest h'] at hc; exact absurd hc (by simp)
  all_goals simp_all
...
theorem netStep_dirac {l : ExtendedLabel P.n M}
    (h : NetworkStep P M G callPayload ghostStep ghostOut s l μ) :
    ∃ s', μ = PMF.pure s' := by
  cases h <;> exact ⟨_, rfl⟩

theorem netStep_gbcaSend {r : ℕ} {j : Fin P.n} {m : M}
    (h : NetworkStep P M G callPayload ghostStep ghostOut s
      (Sum.inr (.gbcaSend r j m)) μ) :
    μ = PMF.pure ((s.gsent r j m).writeGhost ghostStep (Sum.inr (.gbcaSend r j m))) := by
  cases h; rfl

theorem netStep_gbcaDeliver {r : ℕ} {i j : Fin P.n} {m : M}
    (h : NetworkStep P M G callPayload ghostStep ghostOut s
...
theorem netStep_decidedSend {j : Fin P.n} {b : Bool}
    (h : NetworkStep P M G callPayload ghostStep ghostOut s
      (Sum.inr (.decidedSend j b)) μ) :
    b ∉ s.dsent j ∧ μ = PMF.pure (s.dput j b) := by
  cases h; exact ⟨by assumption, rfl⟩

theorem netStep_decidedDeliver {i j : Fin P.n} {b : Bool}
    (h : NetworkStep P M G callPayload ghostStep ghostOut s
...
theorem netStep_byzantineCallGLoop {r : ℕ} {k : Fin P.n} {b : Bool}
    (h : NetworkStep P M G callPayload ghostStep ghostOut s
...
theorem netStep_byzantineCallG {r : ℕ} {k : Fin P.n} {b : Bool}
    (h : NetworkStep P M G callPayload ghostStep ghostOut s
...
theorem netStep_byzantineRetG {r : ℕ} {k : Fin P.n} {out : GbcaOut} {bnd : Bool}
    (h : NetworkStep P M G callPayload ghostStep ghostOut s
...
theorem netStep_tau
  ...
  cases h
  case byzantineG => exact Or.inl ⟨_, _, _, by assumption, rfl⟩
  case byzantineD => exact Or.inr ⟨_, _, by assumption, rfl⟩
...
variable {n : ℕ} {M G : Type}
    {ghostStep : ExtendedLabel n M → NetStateP n M G → G → G}

@[simp] theorem writeGhost_sent (s : NetStateP n M G) (L : ExtendedLabel n M) :
    (s.writeGhost ghostStep L).sent = s.sent := by
  unfold NetStateP.writeGhost; split <;> rfl

@[simp] theorem writeGhost_dsent (s : NetStateP n M G) (L : ExtendedLabel n M) :
    (s.writeGhost ghostStep L).dsent = s.dsent := by
  unfold NetStateP.writeGhost; split <;> rfl

@[simp] theorem writeGhost_F (s : NetStateP n M G) (L : ExtendedLabel n M) :
    (s.writeGhost ghostStep L).F = s.F := by
  unfold NetStateP.writeGhost; split <;> rfl

/-- A label naming no round leaves the whole state where it stands. -/
theorem writeGhost_of_round_none (s : NetStateP n M G) {L : ExtendedLabel n M}
...
theorem writeGhost_ghostRec_self (s : NetStateP n M G) {L : ExtendedLabel n M} {r : ℕ}
    (h : roundOf L = some r) :
...
theorem writeGhost_ghostRec_ne (s : NetStateP n M G) {L : ExtendedLabel n M} {r r' : ℕ}
...
@[simp] theorem writeGhost_unit (s : NetStateP n M Unit) (L : ExtendedLabel n M) :
...
theorem syncN_inv {u : ∀ _ : Fin P.n, ProcRecP P.n S} {l : ExtendedLabel P.n M}
...
theorem systemExtended_event_inv {u : ∀ _ : Fin P.n, ProcRecP P.n S}
    {w : NetStateP P.n M G} {o : ℕ → WCC.SpecState P.n} {e : NetworkEvent P.n M}
    {μ : PMF (State P M S G)}
...
theorem systemExtended_lab_inv {u : ∀ _ : Fin P.n, ProcRecP P.n S}
    {w : NetStateP P.n M G} {o : ℕ → WCC.SpecState P.n} {l : Label P.n}
...
theorem systemExtended_tau_inv {u : ∀ _ : Fin P.n, ProcRecP P.n S}
...
relation admits. -/
theorem systemExtended_retG_bound {u : ∀ _ : Fin P.n, ProcRecP P.n S}
    {w : NetStateP P.n M G} {o : ℕ → WCC.SpecState P.n}
  ...
  have hne : (Label.retG r id out bnd : Label P.n) ≠ Label.tau := by simp
  obtain ⟨x, w', ω, -, hN, -, -⟩ := systemExtended_lab_inv hne h
  exact (netStep_retG hN).1

/-- A composite Byzantine graded-agreement return announces a bit the same
ghost relation admits (D11). -/
theorem systemExtended_byzantineRetG_bound {u : ∀ _ : Fin P.n, ProcRecP P.n S}
    {w : NetStateP P.n M G} {o : ℕ → WCC.SpecState P.n}
  ...
  exact (netStep_byzantineRetG hN).2.1
