...
(`AFW.apIn1` and its three companions). `AFW.storeIn` (`ABA/ImplementationByAFW/System.lean`) is
...
of `ABA/ImplementationByAFW/System.lean`, where the adversary's ghost write reads them. -/
...
def toProc {n : ℕ} (st : StageRec n) : GBCA.ByAFW.ProcRec n where
...
@[simp] theorem procs_toRound (j : Fin P.n) :
    GBCA.ByAFW.procs (toRound P u w r) j = toProc ((u j).2.stage r) := rfl

@[simp] theorem bound_toRound : GBCA.ByAFW.bound (toRound P u w r) = (w.ghostRec r).2.2 := rfl

@[simp] theorem ga1_toRound : GBCA.ByAFW.ga1 (toRound P u w r) = toGa1 P u w r := rfl

@[simp] theorem ga2_toRound : GBCA.ByAFW.ga2 (toRound P u w r) = toGa2 P u w r := rfl
...
def StoreInv (P : Params) (u : ∀ _ : Fin P.n, AFW.ProcRec P.n)
    (w : NetState P.n) : Prop :=
  ∀ (r : ℕ) (k : Fin P.n),
    BRB.Inv P k (Gather.brbIn (GBCA.ByAFW.ga1 (toRound P u w r)) k) ∧
      BRB.Inv P k (Gather.brbBind (GBCA.ByAFW.ga1 (toRound P u w r)) k) ∧
      BRB.Inv P k (Gather.brbIn (GBCA.ByAFW.ga2 (toRound P u w r)) k) ∧
      BRB.Inv P k (Gather.brbBind (GBCA.ByAFW.ga2 (toRound P u w r)) k)
