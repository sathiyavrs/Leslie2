...
exactly the `l₀`-rows of `BRB.ImplStep` (`ABA/ReliableBroadcast/BrachaImplementation.lean`), one
...
(`ABA/ReliableBroadcast/BrachaImplementation.lean`), so the instance and the rule table `ImplStep`
