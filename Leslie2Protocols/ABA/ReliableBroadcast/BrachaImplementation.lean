...
`ImplStep` is the rows of the instance `BRB.implInst`
...
`BRB.implInst_step_iff_row`. It is a relation on the product state; the system is the composition.
...
(`BRB.implInst`, `ABA/ReliableBroadcast/BrachaComposition.lean`), stated over the product state: one
