...
abbrev BrachaState (n : ℕ) (M : Type) : Type := SubState n (PState M) (BMsg M)
...
inductive BrachaStep (P : Params) (ldr : Fin P.n) :
  ...
  | deliver (s : BrachaState P.n M) (i j : Fin P.n) (m : BMsg M)
      (h : m ∈ s.sent j) :
      BrachaStep P ldr s .tau (PMF.pure (s.recvMsg i j m))
  ...
  yet. -/
  | voteAmp (s : BrachaState P.n M) (j : Fin P.n) (m : M)
  ...
  | byzantine (s : BrachaState P.n M) (j : Fin P.n) (m : BMsg M) (h : j ∈ s.F) :
      BrachaStep P ldr s .tau (PMF.pure (s.mcast j m))
