...
(`ABA/Round/Ideal.lean`) forward-simulates the GBCA instance over the gather
specifications (`ABA/Round/Pair.lean`), along the componentwise pairing of the
...
guards feed the pair's fused row. The two sides carry the same core on a
return label (`Gather.CoreRel.core_eq`), so the `link` row writes the same
bound bit on both and the `retG` row announces the same one.
-/
...
theorem idealRel_corrupt {s : IdealState P.n} {t : PairState P.n}
    ...
    IdealRel P (s.1.corruptAll P id, s.2.1.corruptAll P id, s.2.2)
      (t.1.corrupt P id, t.2.1.corrupt P id, t.2.2) :=
  ⟨Gather.coreRel_corrupt hR.1 id, Gather.coreRel_corrupt hR.2.1 id, hR.2.2⟩

/-- Mapping a first-coordinate τ-chain of gather specification steps into the
pair. -/
private theorem chain_map_ga1 {r : ℕ}
    (t2 : Gather.SpecState P.n (Option Bool) × Option Bool)
    ...
    List.IsChain (fun A B => (pairInst P r).LStep A Silent.τ B)
      ((t, t2) :: ts.map (fun x => (x, t2))) :=
  List.isChain_map_of_isChain (fun x => (x, t2)) (fun a b hab => PairStep.ga1Tau (a, t2) b hab) h
...
private theorem chain_map_ga2 {r : ℕ} (t1 : Gather.SpecState P.n Bool)
    (β : Option Bool) {t : Gather.SpecState P.n (Option Bool)}
  ...
  List.isChain_map_of_isChain (fun x => (t1, x, β))
    (fun a b hab => PairStep.ga2Tau (t1, a, β) b hab) h
...
theorem idealRefines (P : Params) (r : ℕ) :
  ...
  | callG id b t1' h =>
    ...
    subst hq₁'
    generalize hμ : (PMF.pure t1' : PMF (Gather.IdealState P.n Bool)) = μ1 at h
    cases h with
    | call id' b' hcall =>
  ...
  | ga2Tau t2' h =>
    ...
    subst hq₁'
    exact ⟨q₂, Or.inl ⟨rfl, System.weakLSilent_refl _ q₂⟩,
      hR.1, Gather.coreRel_tau hR.2.1 h, hR.2.2⟩
  | link id g C t1' h h2 =>
    rw [PMF.mem_support_pure_iff] at hq₁'
    subst hq₁'
    generalize hμ : (PMF.pure t1' : PMF (Gather.IdealState P.n Bool)) = μ1 at h
    ...
    | ret id' g' hin hsub hQ hr =>
      have ht1' := PMF.pure_injective hμ
      subst ht1'
      obtain ⟨ts, hchain, hcore, hmem, hcov, hret1, hRel1⟩ :=
        Gather.retRun hR.1 hin hsub hQ hr
      have hguard2 : q₂.2.1.call id = none := by
        rw [hR.2.1.call_eq id]
        exact h2
      ...
      have hlinkstep : (pairInst P r).LStep
          ...
          ({ ts.getLastD q₂.1 with
           ...
           { q₂.2.1 with
            call := Function.update q₂.2.1.call id (some (cand P g)) },
           some (q₂.2.2.getD
             (boundOfCore P (q₁.1.core.getD (Gather.coreOf P q₁.1.ga))))) := by
        rw [show ((q₂.1, q₂.2) : PairState P.n)
            = (fun x => (x, q₂.2)) q₂.1 from rfl, getLastD_map]
        exact PairStep.link ((ts.getLastD q₂.1), q₂.2) id g _ _
          (Gather.Step.ret _ id g _ hcore hmem hcov hret1) hguard2
      refine ⟨({ ts.getLastD q₂.1 with
          ret := Function.update (ts.getLastD q₂.1).ret id true },
        { q₂.2.1 with call := Function.update q₂.2.1.call id (some (cand P g)) },
        some (q₂.2.2.getD
          (boundOfCore P (q₁.1.core.getD (Gather.coreOf P q₁.1.ga))))),
        Or.inl ⟨rfl, ?_⟩, hRel1,
        Gather.coreRel_call (x := cand P g) hR.2.1 h2, by rw [hR.2.2]⟩
  ...
  | retG id g C t2' h =>
    rw [PMF.mem_support_pure_iff] at hq₁'
    subst hq₁'
    generalize hμ : (PMF.pure t2' : PMF (Gather.IdealState P.n (Option Bool))) = μ2 at h
    cases h with
    | ret id' g' hin hsub hQ hr =>
      have ht2' := PMF.pure_injective hμ
      subst ht2'
      obtain ⟨ts, hchain, hcore, hmem, hcov, hret1, hRel2⟩ :=
        Gather.retRun hR.2.1 hin hsub hQ hr
      ...
      have hlaststep : (pairInst P r).LStep
          ...
          (q₂.1, { ts.getLastD q₂.2.1 with
            ret := Function.update (ts.getLastD q₂.2.1).ret id true },
           q₂.2.2) := by
        rw [show ((q₂.1, q₂.2.1, q₂.2.2) : PairState P.n)
            = (fun x => (q₂.1, x, q₂.2.2)) q₂.2.1 from rfl, getLastD_map,
          ← hR.2.2]
        exact PairStep.retG (q₂.1, ts.getLastD q₂.2.1, q₂.2.2) id g _ _
          (Gather.Step.ret _ id g _ hcore hmem hcov hret1)
  ...
  | fail id =>
    rw [PMF.mem_support_pure_iff] at hq₁'
    subst hq₁'
    exact ⟨(q₂.1.corrupt P id, q₂.2.1.corrupt P id, q₂.2.2),
