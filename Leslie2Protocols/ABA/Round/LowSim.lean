...
`GBCA.lowRefines`: the GBCA implementation (`ABA/Round/Low.lean`)
forward-simulates the GBCA instance over the gather-over-BRB components
(`ABA/Round/Ideal.lean`), along the componentwise pairing of the broadcast
...
private theorem chain_map_ga1 {r : ℕ} (t2 : Gather.IdealState P.n (Option Bool))
  ...
  List.isChain_map_of_isChain (fun x => (x, t2)) (fun a b hab => IdealStep.ga1Tau (a, t2) b hab) h
    ...
    | call id' b' hcall hbcall =>
      ...
      refine ⟨_, Or.inr ⟨by simp, System.weakLStep_of_step (by simp)
        (IdealStep.callG q₂ id b _
          (Gather.IdealStep.call q₂.1 id b (by rw [hR.1.ga_eq]; exact hcall)))⟩,
        Gather.lowRel_call hR.1 hcall hbcall, hR.2⟩
    | callLoop id' b' =>
      have ht1' := PMF.pure_injective hμ
      subst ht1'
      exact ⟨q₂, Or.inr ⟨by simp, System.weakLStep_of_step (by simp)
        (IdealStep.callG q₂ id b q₂.1 (Gather.IdealStep.callLoop q₂.1 id b))⟩, hR⟩
  | ga1Tau t1' h =>
    rw [PMF.mem_support_pure_iff] at hq₁'
    subst hq₁'
