...
`GBCA.PairSubStep` is the rule table of `GBCA.pairSub` (`ABA/Round/Sub.lean`) —
...
`pairSub_step_iff_row` is the row characterisation: at a shared label `l₀`, the
transitions of the round over the labels `GSub.gPull` sends to `l₀` are exactly
the `l₀`-rows of `PairSubStep`, on the same state and with the same
distribution.
...
variable {X : Type} [DecidableEq X] {P : Params} {c c' : Gather.SpecState P.n X}
  {L : RLab P.n} {ψ : RLab P.n → Option (Gather.SubLab P.n X)}
...
theorem liftedSpec_tau_step
    (h : (Gather.liftedSpec P X).step c (Silent.τ : Gather.SubLab P.n X) (PMF.pure c')) :
...
theorem row_liftedSpec_tau_step (h : Gather.Step P c Gather.Lab.tau (PMF.pure c')) :
    (Gather.liftedSpec P X).step c (Silent.τ : Gather.SubLab P.n X) (PMF.pure c') :=
...
/-- The rows of the round over the gather specifications (`GBCA.pairSub`),
stated over the round's state: one constructor per case of
`GBCA.pairSub_step_iff_row`. All transitions are Dirac. -/
inductive PairSubStep (P : Params) (r : ℕ) :
    PairSubState P.n → Lab P.n → PMF (PairSubState P.n) → Prop
  /-- The call arrives: program `id` records the input and the first gather
  takes the call. -/
  | callG (s : PairSubState P.n) (id : Fin P.n) (b : Bool)
      (t1 : Gather.SpecState P.n Bool) (h0 : (procs s id).input = none)
      (h : Gather.Step P (ga1 s) (.call id b) (PMF.pure t1)) :
      PairSubStep P r s (.callG r id b)
  ...
  | callLoop (s : PairSubState P.n) (id : Fin P.n) (b : Bool)
      (t1 : Gather.SpecState P.n Bool)
      (h : Gather.Step P (ga1 s) (.call id b) (PMF.pure t1)) :
      PairSubStep P r s (.callG r id b) (PMF.pure (setGa1 s t1))
  /-- An internal step of the first gather. -/
  | ga1Tau (s : PairSubState P.n) (t1 : Gather.SpecState P.n Bool)
      (h : Gather.Step P (ga1 s) Gather.Lab.tau (PMF.pure t1)) :
      PairSubStep P r s .tau (PMF.pure (setGa1 s t1))
  /-- An internal step of the second gather. -/
  | ga2Tau (s : PairSubState P.n) (t2 : Gather.SpecState P.n (Option Bool))
      (h : Gather.Step P (ga2 s) Gather.Lab.tau (PMF.pure t2)) :
      PairSubStep P r s .tau (PMF.pure (setGa2 s t2))
  ...
  unwritten. -/
  | ret1 (s : PairSubState P.n) (id : Fin P.n) (g : Fin P.n → Option Bool)
  ...
  | call2 (s : PairSubState P.n) (id : Fin P.n) (x : Option Bool)
  ...
  | ret2 (s : PairSubState P.n) (id : Fin P.n) (g : Fin P.n → Option (Option Bool))
...
/-- **The projection.** -/
theorem pairSub_step_row (P : Params) (r : ℕ) :
    ∀ (s : PairSubState P.n) (l : NLab P.n) (μ : PMF (PairSubState P.n)),
      (pairSub P r).step s l μ →
      ∃ l₀, GSub.gPull P.n l = some l₀ ∧ PairSubStep P r s l₀ μ := by
  ...
  rcases (roundSubAt_step_iff P r (Gather.liftedSpec P Bool)
  ...
  · obtain ⟨x, v', c', d', rfl, hlayer, hga1, hga2⟩ :=
    ...
    | ret1 id g C =>
      ...
      exact PairSubStep.ret1 _ id g C c' hin hc hg1
    | call2 id y =>
      ...
      exact PairSubStep.call2 _ id y d' hc h2' hg2
    | ret2 id g C =>
      ...
      subst hx; subst hv; subst hc1
      exact PairSubStep.ret2 _ id g C d' h2' ho hg2
  · by_cases hlτ : l = Sum.inl Lab.tau
    · subst hlτ
      refine ⟨Lab.tau, rfl, ?_⟩
      rcases roundPreAt_tau_inv h1 h2 hlab with ⟨c', rfl, hs⟩ | ⟨d', rfl, hs⟩
      · exact PairSubStep.ga1Tau _ c' (liftedSpec_tau_step hs)
      · exact PairSubStep.ga2Tau _ d' (liftedSpec_tau_step hs)
    · obtain ⟨x, v', c', d', rfl, hlayer, hga1, hga2⟩ :=
      ...
      | inl l₀ =>
        ...
        | callG r' id b =>
          ...
          have hd : d' = d := Gather.lift_step_none (by simp) hga2
          subst hx; subst hv; subst hd
          exact ⟨_, rfl, PairSubStep.callG _ id b c' h0 hg1⟩
        | retG r' id out bnd =>
          obtain ⟨hproc, hnet⟩ :=
          ...
          exact ⟨_, rfl, PairSubStep.retG _ id out ho hr⟩
        | fail id =>
          ...
          exact ⟨_, rfl, PairSubStep.fail _ id⟩
      | inr ev =>
        cases ev with
        ...
        | gcallLoop r' id b =>
          ...
          have hd : d' = d := Gather.lift_step_none (by simp) hga2
          subst hx; subst hv; subst hd
          exact ⟨_, rfl, PairSubStep.callLoop _ id b c' hg1⟩
        | byzCallG r' k b =>
          ...
          have hd : d' = d := Gather.lift_step_none (by simp) hga2
          subst hx; subst hv; subst hd
          exact ⟨_, rfl, PairSubStep.callG _ k b c' h0 hg1⟩
        | byzCallGLoop r' k b =>
          obtain ⟨hproc, hnet⟩ :=
          ...
          have hd : d' = d := Gather.lift_step_none (by simp) hga2
          subst hx; subst hv; subst hd
          exact ⟨_, rfl, PairSubStep.callLoop _ k b c' hg1⟩
        | byzRetG r' k out bnd =>
          obtain ⟨hproc, hnet⟩ :=
          ...
          have hd : d' = d := Gather.lift_step_none (by simp) hga2
          subst hx; subst hv'; subst hc1; subst hd
          exact ⟨_, rfl, PairSubStep.retG _ k out ho hr⟩

/-- **The embedding.** -/
theorem row_pairSub_step (P : Params) (r : ℕ) :
    ∀ (s : PairSubState P.n) (l₀ : Lab P.n) (μ : PMF (PairSubState P.n)),
      PairSubStep P r s l₀ μ →
      ∃ l, GSub.gPull P.n l = some l₀ ∧ (pairSub P r).step s l μ := by
  ...
  | callG id b t1 h0 h =>
    ...
    exact ⟨Sum.inl (.callG r id b), rfl, roundSubAt_lab_step (by simp) hlayer hg1 hg2⟩
  | callLoop id b t1 h =>
    ...
    exact ⟨Sum.inr (.gcallLoop r id b), rfl, roundSubAt_lab_step (by simp) hlayer hg1 hg2⟩
  | ga1Tau t1 h =>
    exact ⟨Sum.inl Lab.tau, rfl, roundSubAt_tau_ga1 (row_liftedSpec_tau_step h)⟩
  | ga2Tau t2 h =>
    exact ⟨Sum.inl Lab.tau, rfl, roundSubAt_tau_ga2 (row_liftedSpec_tau_step h)⟩
  | ret1 id g C t1 hin hc h =>
    ...
    exact ⟨Sum.inl Lab.tau, rfl, roundSubAt_event_step _ hlayer hg1 hg2⟩
  ...
  | retG id out ho hr =>
    have hlayer : (layer P r).step (u, v)
        ...
        (PMF.pure (Function.update u id { u id with returned := true }, v)) :=
    ...
    exact ⟨Sum.inl (.retG r id out (v.getD (boundOfCore P ∅))), rfl,
      roundSubAt_lab_step (by simp) hlayer hg1 hg2⟩
  | fail id =>
    ...
    exact ⟨Sum.inl (.fail id), rfl, roundSubAt_lab_step (by simp) hlayer hg1 hg2⟩
...
of `PairSubStep`, on the same state and with the same distribution. -/
theorem pairSub_step_iff_row (P : Params) (r : ℕ) (s : PairSubState P.n) (l₀ : Lab P.n)
    (μ : PMF (PairSubState P.n)) :
    (∃ l, GSub.gPull P.n l = some l₀ ∧ (pairSub P r).step s l μ) ↔ PairSubStep P r s l₀ μ := by
  constructor
  · rintro ⟨l, hl, hstep⟩
    obtain ⟨l₁, hl₁, hrow⟩ := pairSub_step_row P r s l μ hstep
    have hll : l₁ = l₀ := Option.some.inj (show (some l₁ : Option (Lab P.n)) = some l₀ by
      rw [← hl₁, hl])
    subst hll
    exact hrow
  · exact row_pairSub_step P r s l₀ μ

/-- info: 'PLTS.ABA.GBCA.pairSub_step_iff_row' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms pairSub_step_iff_row
