...
`GBCA.pairRefines`: the round over the gather specifications
(`GBCA.pairInst`, `ABA/Round/Sub.lean`) forward-simulates the graded agreement
specification read over the round's interface (`GSub.liftedSpec`), along
`GBCA.PairRel`.

A transition of the round is one row of `GBCA.PairStep`
(`GBCA.pairInst_step_row`), the row is answered by a weak run of the
...
theorem pairRel_init (P : Params) (r : ℕ) :
    PairRel P ((pairInst P r).init) ((GSub.liftedSpec P r).init) := by
...
interface. A transition of the round is one row of `GBCA.PairStep`
(`GBCA.pairInst_step_row`), the row is answered by a weak run of the
specification (`pairRel_row`), and that run is lifted to the interface along a
section of `GSub.gPull`. -/
theorem pairRefines (P : Params) (r : ℕ) :
    ForwardSimulation (pairInst P r) (GSub.liftedSpec P r) (PairRel P) := by
  constructor
...
/-- info: 'PLTS.ABA.GBCA.pairRel_corrupt' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
