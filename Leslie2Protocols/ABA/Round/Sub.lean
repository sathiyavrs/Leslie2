...
(`GBCA.ProcRec`). Its guards read that record and nothing else.
...
The round speaks `NLab n` natively, as `GSub.sub` does. The call loop of the
...
(`GSub.isFailN`), applied to every round at once, and `GBCA.corruptAll` is the
...
variable {n : ℕ} (r : ℕ) (id k i j : Fin n) (b c bnd : Bool) (x : Option Bool)
  (out : GbcaOut) (m : GBCA.Msg) (g : Fin n → Option Bool)
