...
Byzantine row `Sum.inr (.byzRetG r id out bnd)` alike, `GSub.gPull` sending
...
def BindingTraceN (P : Params) (r : ℕ) (t : Seq (NLab P.n)) : Prop :=
  (∀ (l₁ l₂ : NLab P.n) (id₁ id₂ : Fin P.n) (o₁ o₂ : GbcaOut) (β₁ β₂ : Bool),
      l₁ ∈ t → l₂ ∈ t → GSub.gPull P.n l₁ = some (Lab.retG r id₁ o₁ β₁) →
      GSub.gPull P.n l₂ = some (Lab.retG r id₂ o₂ β₂) → β₁ = β₂) ∧
    ∀ (l : NLab P.n) (id : Fin P.n) (o : GbcaOut) (β v : Bool),
      l ∈ t → GSub.gPull P.n l = some (Lab.retG r id o β) →
      outValue o = some v → v = β

/-! ### Transitions of the lifted specification -/

/-- A transition of the lifted specification on a label `GSub.gPull` names is a
transition of the specification at that label. -/
theorem liftedSpec_step_some {s : SpecState P.n} {l : NLab P.n} {l₀ : Lab P.n}
    {μ : PMF (SpecState P.n)} (hpull : GSub.gPull P.n l = some l₀)
    (h : (GSub.liftedSpec P r).step s l μ) : Step P r s l₀ μ :=
  (System.mapIdle_step_some (sys := specInst P r) hpull μ).mp h

/-- A transition of the lifted specification is a transition of the
specification at the label `GSub.gPull` names, or a self-loop. -/
theorem liftedSpec_step_cases {s s' : SpecState P.n} {l : NLab P.n}
    {μ : PMF (SpecState P.n)} (h : (GSub.liftedSpec P r).step s l μ)
    (hs' : s' ∈ μ.support) :
    (∃ l₀, GSub.gPull P.n l = some l₀ ∧ Step P r s l₀ μ) ∨ s' = s := by
  rcases (System.mapIdle_step (GSub.gPull P.n) (specInst P r) s l μ).mp h with
    ⟨l₀, hpull, hstep⟩ | ⟨-, rfl⟩
...
theorem liftedSpec_excluded_mem_stable {e : AlterSeq (SpecState P.n) (NLab P.n)}
    (he : is_exec e (GSub.liftedSpec P r)) {k₁ k₂ : ℕ} (hk : k₁ ≤ k₂)
    {s₁ s₂ : SpecState P.n} {b : Bool}
    (hst₁ : e.stateAt k₁ = some s₁) (hst₂ : e.stateAt k₂ = some s₂)
    (hb : b ∈ s₁.excluded) : b ∈ s₂.excluded :=
  is_exec_stable (sys := GSub.liftedSpec P r) (fun s => b ∈ s.excluded)
...
theorem liftedSpec_excluded_card_le_one {e : AlterSeq (SpecState P.n) (NLab P.n)}
    (he : is_exec e (GSub.liftedSpec P r)) {k : ℕ} {s : SpecState P.n}
    (hst : e.stateAt k = some s) : s.excluded.card ≤ 1 :=
  is_exec_stable (sys := GSub.liftedSpec P r) (fun s => s.excluded.card ≤ 1)
...
private theorem liftedSpec_excluded_eq_of_mem
    {e : AlterSeq (SpecState P.n) (NLab P.n)} (he : is_exec e (GSub.liftedSpec P r))
...
theorem liftedSpec_retG_bound_agree {e : AlterSeq (SpecState P.n) (NLab P.n)}
    (he : is_exec e (GSub.liftedSpec P r)) {k₁ k₂ : ℕ} {s₁ s₂ : SpecState P.n}
...
in the support of every achievable trace distribution of `GSub.liftedSpec P r`
satisfies `BindingTraceN`. -/
theorem liftedSpec_binding (P : Params) (r : ℕ) :
    ∀ D ∈ achievableTraceDists (GSub.liftedSpec P r), ∀ t, D t ≠ 0 →
