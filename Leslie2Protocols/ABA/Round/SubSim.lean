...
`GBCA.Sub.pairRefines`: the round over the gather specifications
(`GBCA.pairSub`, `ABA/Round/Sub.lean`) forward-simulates the graded agreement
specification read over the round's interface (`GSub.liftedSpec`), along
`GBCA.Sub.PairRel`.

A transition of the round is one row of `GBCA.PairSubStep`
(`GBCA.pairSub_step_row`), the row is answered by a weak run of the
specification (`pairRel_row`), and that run is lifted to the interface along a
section of `GSub.gPull`.
...
def OutCert (P : Params) (s : PairSubState P.n) : GbcaOut → Prop
...
it. -/
theorem OutCert.mono {s s' : PairSubState P.n} (h1 : (ga1 s').core = (ga1 s).core)
...
structure PairInv (P : Params) (s : PairSubState P.n) : Prop where
...
theorem PairInv.initial (P : Params) (r : ℕ) : PairInv P ((pairSub P r).init) := by
  refine ⟨?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_⟩ <;>
    simp [pairSub_init, procs, bound, ga1, ga2, Gather.SpecState.initial, ProcRec.initial]

/-- The invariant is preserved by every row. -/
theorem PairInv.step {r : ℕ} {s : PairSubState P.n} {l : Lab P.n}
    {μ : PMF (PairSubState P.n)} (hInv : PairInv P s) (hstep : PairSubStep P r s l μ)
    {s' : PairSubState P.n} (hs' : s' ∈ μ.support) : PairInv P s' := by
  cases hstep with
  | callG id b t1 h0 h =>
...
def ExcludedEv (P : Params) (s : PairSubState P.n) (b : Bool) : Prop :=
  ∃ S, (ga1 s).core = some S ∧ APSet.cnt S b < S.card - P.f

/-- The refinement relation of the round over the gather specifications. -/
structure PairRel (P : Params) (s : PairSubState P.n) (t : GBCA.SpecState P.n) : Prop where
  /-- The invariant. -/
  inv : PairInv P s
...
theorem pairRel_init (P : Params) (r : ℕ) :
    PairRel P ((pairSub P r).init) ((GSub.liftedSpec P r).init) := by
  refine ⟨PairInv.initial P r, ?_, ?_, ?_, ?_, ?_, ?_, ?_⟩ <;>
    simp [pairSub_init, procs, bound, ga1, ga2, Gather.SpecState.initial,
      GBCA.SpecState.initial, ProcRec.initial]

/-- **Broadcast compatibility**: the relation is preserved by corrupting both
sides at once. -/
theorem pairRel_corrupt {r : ℕ} {s : PairSubState P.n} {t : GBCA.SpecState P.n}
    (hR : PairRel P s t) (id : Fin P.n) :
    PairRel P (corruptAll P id (Gather.SpecState.corrupt P) (Gather.SpecState.corrupt P) s)
      (t.corrupt P id) := by
  refine ⟨hR.inv.step (r := r) (PairSubStep.fail s id) (by rw [PMF.mem_support_pure_iff]),
    ?_, ?_, ?_, ?_, ?_, ?_, ?_⟩
  · intro k
...
theorem pairRel_row (P : Params) (r : ℕ) (q₁ : PairSubState P.n)
    (q₂ : GBCA.SpecState P.n) (hR : PairRel P q₁ q₂) (l₀ : Lab P.n)
    (μ : PMF (PairSubState P.n)) (hrow : PairSubStep P r q₁ l₀ μ)
    (q₁' : PairSubState P.n) (hq₁' : q₁' ∈ μ.support) :
  ...
  | retG id out ho hr =>
    ...
    have hret_eq : ∀ k, Function.update q₂.ret id true k =
        (procs (setProcs q₁ (Function.update (procs q₁) id
          { procs q₁ id with returned := true })) k).returned := by
      intro k
...
interface. A transition of the round is one row of `GBCA.PairSubStep`
(`GBCA.pairSub_step_row`), the row is answered by a weak run of the
specification (`pairRel_row`), and that run is lifted to the interface along a
section of `GSub.gPull`. -/
theorem pairRefines (P : Params) (r : ℕ) :
    ForwardSimulation (pairSub P r) (GSub.liftedSpec P r) (PairRel P) := by
  constructor
  intro q₁ q₂ hR l μ hstep q₁' hq₁'
  obtain ⟨l₀, hpull, hrow⟩ := pairSub_step_row P r q₁ l μ hstep
  obtain ⟨t', hdis, hrel⟩ := pairRel_row P r q₁ q₂ hR l₀ μ hrow q₁' hq₁'
  refine ⟨t', ?_, hrel⟩
  rcases hdis with ⟨hτ, hweak⟩ | ⟨hτ, hweak⟩
  · exact Or.inl ⟨GSub.gPull_eq_tau (by rw [hpull, hτ]; rfl),
      GSub.weakLSilent_liftedSpec P r hweak⟩
  · refine Or.inr ⟨?_, GSub.weakLStep_liftedSpec P r hτ hpull hweak⟩
    intro hl
    refine hτ ?_
    have h2 : GSub.gPull P.n (Silent.τ : Net.NLab P.n) = some l₀ := by
      rw [← hl]; exact hpull
    rw [GSub.gPull_tau] at h2
    exact (Option.some.inj h2).symm

/-! ### Mechanical axiom check -/

/-- info: 'PLTS.ABA.GBCA.Sub.pairRefines' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms pairRefines

/-- info: 'PLTS.ABA.GBCA.Sub.pairRel_corrupt' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
