...
inductive Step (P : Params) (r : ℕ) :
  ...
  | callResolve (s : SpecState P.n) (id : Fin P.n) (h : s.called id = false)
      ...
      Step P r s (.callW r id)
        (P.wccPMF.map (fun o => { s.record id with val := o.toTVal }))
...
theorem step_callW_inv {P : Params} {r : ℕ} {s : SpecState P.n} {id : Fin P.n}
    ...
    μ = PMF.pure s ∨
      ...
      (s.called id = false ∧ s.val = .bot ∧ (s.record id).threshold P ∧
        μ = P.wccPMF.map (fun o => { s.record id with val := o.toTVal })) := by
  cases h with
...
theorem step_tau_inv {P : Params} {r : ℕ} {s : SpecState P.n}
...
theorem step_callW_support {P : Params} {r : ℕ} {s : SpecState P.n} {id : Fin P.n}
    {μ : PMF (SpecState P.n)} (h : Step P r s (.callW r id) μ)
