% Round-loop per-process phase machine (PLTS.ABA.Comp.CoreProcStepN). Automata style.
\begin{tikzpicture}[
  ...
  \path
    ...
    (idle) edge[loop above, looseness=5]
      node[lbl, above]{\texttt{inputLoop}} (idle);
  % round back-edge, routed over the top so it clears the DECIDED node below
  \draw[->] (aww) to[out=115, in=65, looseness=0.55]
