...
% Four control states keyed on (mode, val), each named by its mode. Rule 3 is drawn as a
...
% drawn; fail (D1) and the corrupted interface callByzantine/retByzantine (D23) are not,
% none of them writing the mode. See the caption.
\begin{tikzpicture}[
    ->, >={Stealth[round]}, auto, semithick, on grid,
    every state/.style={draw, rectangle, rounded corners, minimum size=1.15cm,
  ...
  \node[state, initial, initial text={}] (idl) at (0,0)      {$flipEnabled$\\$val{=}\bot$};
  \node[branch]                          (br)  at (3.9,0)    {};
  \node[state]                           (dea) at (7.4,2.0)  {$noRuleEnabled$};
  \node[state]                           (loc) at (7.4,-2.0) {$decisionEnabled$};
  \node[state, accepting]                (dec) at (3.9,-4.5) {$flipEnabled$\\$val$ set};
  ...
  \path
    (br) edge[bend right=14] node[plbl, left]{$\delta_{\mathrm f}$} (dea)
    (br) edge[bend left=14]  node[plbl, right]{$\varepsilon$} (loc)
    (br) edge[bend right=55, looseness=1.3]
      node[plbl, above]{$1{-}\varepsilon{-}\delta_{\mathrm f}$} (idl);
