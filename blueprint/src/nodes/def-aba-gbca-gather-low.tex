\begin{definition}[The gather-based GBCA implementation]
  ...
  \uses{def:aba-gbca-pair, def:aba-gather-mid, def:aba-gather-low}
  The two-gather round at its two concrete readings, each carrying the rule table of Definition~\ref{def:aba-gbca-pair} with the gather coordinates read one level down.
  The instance over gather-over-broadcast components pairs two states of Definition~\ref{def:aba-gather-mid}; the fully concrete instance pairs two states of Definition~\ref{def:aba-gather-low} --- the two gather ladders and, beneath them, the $4n$ Bracha instances carrying the inputs and the $\mathrm{BIND}$ payloads.
