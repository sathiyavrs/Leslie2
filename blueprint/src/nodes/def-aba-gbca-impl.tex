\begin{definition}[The GBCA implementation instance]
  ...
  \lean{PLTS.ABA.GBCA.ByABDY.boundOf}\lean{PLTS.ABA.GBCA.ByABDY.implInst}\leanok
  ...
  It is the pair of the $n$ stage records of Definition~\ref{def:aba-procnet} and the round's network state of Definition~\ref{def:aba-gbcasub}: the network is a component of the state rather than a field of it, and the four names $proc$, $sent$, $recv$ and $F$ used below are accessors on that pair.
  Full rule table: \leandecl{PLTS.ABA.GBCA.ByABDY.implInst}.
\end{definition}
