\begin{definition}[The coin specification]
  ...
  \leandeclnamed{callResolve}{PLTS.ABA.WCC.Step.callResolve} takes the caller whose access carries the count above $f$ at $val = \bot$, records it, and draws $val$ by the distribution of Definition~\ref{def:aba-params}, the one both coin resolutions of the development share.
  ...
  A third row \leandeclnamed{callLoop}{PLTS.ABA.WCC.Step.callLoop} answers a call at every state and records no caller, which makes the call label input-enabled: the crossing access is therefore a recorded one, and the scheduler may defer the resolution past any number of calls.
