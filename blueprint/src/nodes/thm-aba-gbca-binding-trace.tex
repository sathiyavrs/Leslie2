\begin{theorem}[Binding of graded agreement, on a trace]
  ...
  \lean{PLTS.ABA.GBCA.BoundTrace}\lean{PLTS.ABA.GBCA.BindingTrace}\lean{PLTS.ABA.GBCA.specInst_binding}\lean{PLTS.ABA.GBCA.BindingTrace.value_agree}\lean{PLTS.ABA.GBCA.implInst_binding}\lean{PLTS.ABA.GBCA.pairInst_binding}\lean{PLTS.ABA.GBCA.lowPairInst_binding}\leanok
  \uses{def:aba-gbca-spec, thm:gbca-spec-binding, thm:trace-support, thm:forward-sound, thm:gbca-inst-refines, thm:gbca-pair-refines, thm:gbca-gather-refines}
  ...
  The predicate is stated on the labels of a trace, so every inclusion of achievable trace distributions into the instance carries it, and three systems below Transition System~2 satisfy it for that reason.
...
\begin{proof}\leanok
  ...
  The three statements below Transition System~2 are that transport applied to the soundness inclusion of each refinement, by Theorem~\ref{thm:forward-sound}.
\end{proof}
