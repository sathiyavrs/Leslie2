\begin{theorem}[Two-gather round refinement]
  ...
  \lean{PLTS.ABA.GBCA.count_aboveThreshold_of_subMap}\lean{PLTS.ABA.GBCA.boundOfCore_of_aboveThreshold}\lean{PLTS.ABA.GBCA.count_boundOfCore_belowThreshold}\lean{PLTS.ABA.GBCA.ByAFW.PairInv}\lean{PLTS.ABA.GBCA.ByAFW.ExcludedEv}\lean{PLTS.ABA.GBCA.ByAFW.PairRel}\lean{PLTS.ABA.GBCA.ByAFW.pairRefines}\leanok
...
\begin{proof}\leanok
  ...
  A candidate $v$ arriving at the second gather is heavy in the first core, recorded by the invariant at the first gather's return that computed it and read again at the second gather's call, so $v$ is the bound bit $\mathrm{boundOfCore}$ names and a standing $\mathrm{ExcludedEv}(v)$ is refuted.
